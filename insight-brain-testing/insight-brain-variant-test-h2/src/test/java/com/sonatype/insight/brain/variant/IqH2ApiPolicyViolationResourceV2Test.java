...
@IqH2Test
class IqH2ApiPolicyViolationResourceV2Test
{
  ...
  void testGetPolicyViolations() throws Exception {
    ...
    Policy orgPolicy = ctx.tempEntity().newPolicy(org);
    PolicyEvaluation pe1App1 = ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanId1App1");
    PolicyViolation pv1App1 = ctx.tempEntity().newPolicyViolation(pe1App1, orgPolicy, "g1", "a1", "v1", "h1", "r1");
  ...
  @Test
  void testGetCrossStagePolicyViolationById() throws Exception {
    Date date = new Date();
    Organization org = ctx.tempEntity().newOrganization();
    Application app = ctx.tempEntity().newApplication(org.getId());
    Policy orgPolicy = ctx.tempEntity().newPolicy(org);
    PolicyEvaluation pe1App1 = ctx.tempEntity()
  ...
  @Test
  void testGetCrossStagePolicyViolationByConstituentId() throws Exception {
    Date baseDate = new Date();
    Date later = new Date(baseDate.getTime() + 1);
    Organization org = ctx.tempEntity().newOrganization();
    Application app = ctx.tempEntity().newApplication(org.getId());
    Policy orgPolicy = ctx.tempEntity().newPolicy(org);
    PolicyEvaluation evaluation1 = ctx.tempEntity()
        .newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanId1App1", false,
            false, baseDate);
    PolicyViolation violation1 =
        ctx.tempEntity().newPolicyViolation(evaluation1, orgPolicy, "g1", "a1", "v1", "h1", "r1");
    // Equivalent, opened while violation1 was still open
    PolicyEvaluation evaluation2 = ctx.tempEntity()
        .newPolicyEvaluation(app.getId(), DevelopStageType.ID, "scanId2App1", false, false, later);
    ctx.tempEntity().newPolicyViolation(evaluation2, orgPolicy, "g1", "a1", "v1", "h1", "r1");
  ...
  @Test
  void testGetApplicableWaivers() throws Exception {
    DateTime now = DateTime.now();
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(newApp.getId(), BuildStageType.ID, "scanId");
    Policy policy = ctx.tempEntity().newPolicy(newOrg);
    Policy policy2 = ctx.tempEntity().newPolicy(newApp);
    ...
    PolicyViolation violation = ctx.tempEntity().newPolicyViolation(evaluation, policy, identifier, "hash");
    ...
    ctx.tempEntity()
        .newWaiver("hashX", policyId, orgId, constraintFacts, packageUrlAllVersionsWaiver, ALL_VERSIONS, "",
            now.minusDays(10).toDate());
    ctx.tempEntity()
        .newWaiver(null, policyId, orgId, constraintFacts, ALL_COMPONENTS, "", now.minusDays(9).toDate(), null);
    ctx.tempEntity()
        .newWaiver("hash", policyId, appId, constraintFacts, packageUrlAllVersionsWaiver, EXACT_COMPONENT, "",
            now.minusDays(8).toDate(), expiredExpiryTime); // expired
    ctx.tempEntity()
        .newWaiver(null, policyId, appId, constraintFacts, ALL_COMPONENTS, "A comment", now.minusDays(7).toDate(),
            expiringInFutureExpiryTime); // expiring in the future
    ctx.tempEntity()
        .newWaiver("hash2", policy2Id, appId, null, packageUrlAllVersionsWaiver2, ALL_VERSIONS, "",
            now.minusDays(2).toDate());
    ctx.tempEntity()
        .newWaiver(null, policy2Id, appId, null, ALL_COMPONENTS, "", now.minusDays(1).toDate(),
            now.plusMinutes(1).toDate());
    ctx.tempEntity()
        .newWaiver("hash", policy2Id, appId, constraintFacts, packageUrlAllVersionsWaiver, EXACT_COMPONENT, "",
  ...
  @Test
  void testGetApplicableWaiverRequests() throws Exception {
    Date now = new Date();
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization org = ctx.tempEntity().newOrganization();
    Application app = ctx.tempEntity().newApplication(org.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanId");
    Policy policyOrg = ctx.tempEntity().newPolicy(org);
    Policy policyApp = ctx.tempEntity().newPolicy(app);
    ComponentIdentifier identifier =
        ComponentIdentifier.createMavenCoordinates("group", "artifact", "1.0", "c1", "jar");
    ComponentIdentifier identifierForAllVersionsWaiver =
        ComponentIdentifier.createMavenCoordinates("group", "artifact", "2.0", "c1", "jar");
    ...
    PolicyViolation violation = ctx.tempEntity().newPolicyViolation(evaluation, policyOrg, identifier, "hash");
    violation.setConstraintFacts(constraintFacts);
    policyViolationDAO.update(violation);
    ...
    PolicyWaiverRequest policyWaiverRequest1 =
        ctx.tempEntity()
            .newPolicyWaiverRequest(new PolicyWaiverRequest().setHash("hashX")
    ...
    PolicyWaiverRequest policyWaiverRequest2 = ctx.tempEntity()
        .newPolicyWaiverRequest(new PolicyWaiverRequest()
            .setHash(null)
            .setPolicyId(policyIdOrg)
            .setOwnerId(orgId)
            .setConstraintFacts(constraintFacts)
    ...
    PolicyWaiverRequest policyWaiverRequest3 = ctx.tempEntity()
        .newPolicyWaiverRequest(new PolicyWaiverRequest()
            .setHash("hash")
            .setPolicyId(policyIdOrg)
            .setOwnerId(appId)
    ...
    PolicyWaiverRequest policyWaiverRequest4 =
        ctx.tempEntity()
            .newPolicyWaiverRequest(new PolicyWaiverRequest().setHash(null)
                .setPolicyId(policyIdOrg)
                .setOwnerId(appId)
                .setConstraintFacts(constraintFacts)
                .setComponentMatchStrategy(ALL_COMPONENTS)
  ...
  @Test
  void testGetSimilarWaivers() throws Exception {
    DateTime now = DateTime.now();
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    ...
    ComponentIdentifier identifierForAllVersionsWaiver =
        ComponentIdentifier.createMavenCoordinates("group", "artifact", "2.0", "c1", "jar");
    String packageUrlAllVersionsWaiver = PackageUrlIdentifier.toPackageUrl(identifierForAllVersionsWaiver);
    PolicyViolation policyViolation = ctx.tempEntity().newPolicyViolation(evaluation, policy, identifier, "hash");
  ...
  @Test
  void testGetTransitivePolicyViolationsByOwnerStageComponent() throws Exception {
    ...
    ReportTestUtils.createReportFile(application.getId(), scanId,
        zipReportDir("/ApiPolicyViolationResourceV2Test/report", ctx.tempFolder()),
  ...
  @Test
  void testGetTransitivePolicyViolationsByAppScanComponent() throws Exception {
    Application application = ctx.tempEntity().newApplicationWithParent();
    String scanId = "scanId";
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(application.getId(), BuildStageType.ID, scanId);
  ...
  @Test
  void testGetTransitivePolicyViolationsByOwnerStageComponent_FeatureDisabled() throws Exception {
  ...
  @Test
  void testGetTransitivePolicyViolationsByAppScanComponent_FeatureDisabled() throws Exception {
    SystemConfigurationPropertyFeature.INNER_SOURCE_TRANSITIVE_WAIVER.setEnabled(false);
    try {
      HttpResponse response = ctx.restRequest()
          .path(PublicApiPaths.POLICY_VIOLATION_RESOURCE_PATH_V2,
  ...
  @Test
  void testGetApplicableAutoWaiver() throws Exception {
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(newApp.getId(), BuildStageType.ID, "scanId");
    ...
    AutoPolicyWaiver autoPolicyWaiver = ctx.tempEntity().newAutoPolicyWaiver(ownerId);
    PolicyViolation violation = ctx.tempEntity().newPolicyViolation(evaluation, policy, identifier, "hash");
    violation.setConstraintFacts(constraintFacts);
  ...
  @Test
  void testGetApplicableAutoWaiver_NoAutoPolicyWaiverApplied() throws Exception {
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(newApp.getId(), BuildStageType.ID, "scanId");
  ...
  @Test
  void testGetApplicableAutoWaiver_whenExclusionAppliedOnAppLevelAutoPolicyWaiver() throws Exception {
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(newApp.getId(), BuildStageType.ID, "scanId");
  ...
  @Test
  void testGetApplicableAutoWaiver_whenExclusionAppliedOnOrgLevelAutoPolicyWaiver() throws Exception {
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(newApp.getId(), BuildStageType.ID, "scanId");
  ...
  // ALL VERSIONS
  @Test
  void testGetApplicableAutoWaiver_ALL_VERSION_whenExclusionAppliedOnAppLevelAutoPolicyWaiver() throws Exception {
    List<ConstraintFact> constraintFacts = ctx.tempEntity().createArbitraryConstraintFacts();
    Organization newOrg = ctx.tempEntity().newOrganization("NewOrg");
    Application newApp = ctx.tempEntity().newApplication("NewApp", "AppPublicId", newOrg.getId());
    PolicyEvaluation evaluation = ctx.tempEntity().newPolicyEvaluation(newApp.getId(), BuildStageType.ID, "scanId");
  ...
  @Test
  void testGetApplicableAutoWaiver_AutoPolicyWaiverNotEnabled() throws Exception {
  ...
  @Test
  void testGetPolicyViolationsWithDifferentTypes() throws Exception {
    Organization org = ctx.tempEntity().newOrganization();
    Application app = ctx.tempEntity().newApplication(org.getId());
    Policy orgPolicy = ctx.tempEntity().newPolicy(org);
    ...
    PolicyViolation activePv = ctx.tempEntity().newPolicyViolation(pe1App1, orgPolicy, "g1", "a1", "v1", "h1", "r1");

    PolicyViolation waivedPv = ctx.tempEntity().newPolicyViolation(pe1App1, orgPolicy, "g2", "a2", "v2", "h2", "r2");
    waivedPv.setWaiveTime(new Date());
    policyViolationDAO.update(waivedPv);

    PolicyViolation legacyPv = ctx.tempEntity().newLegacyPolicyViolation(pe1App1, orgPolicy);
