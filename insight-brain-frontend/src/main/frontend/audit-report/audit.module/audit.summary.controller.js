 ...
 */
export default function AuditSummaryController(
  $scope,
  $http,
  $window,
  OwnerContext,
  ...
  $scope.doLoad = function () {
    ...
    return $http
      .get(CLMLocations.getAuditReportSummary(OwnerContext.ownerId))
      .then(
        function (response) {
          ...
          $scope.percentKnownComponents = data.totalComponentCount
            ? Math.round(
                (100 * data.knownComponentCount) / data.totalComponentCount
              )
            : 0;
          ...
          $scope.policyViolationCount =
            data.criticalComponentCount +
            data.severeComponentCount +
            data.moderateComponentCount;
        },
        function (error) {
          $scope.loadActive = false;
          $scope.error = error;
