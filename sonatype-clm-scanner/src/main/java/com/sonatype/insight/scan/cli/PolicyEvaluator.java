...
public class PolicyEvaluator
{
    ...
    private void validateServerAccess( Parameters params, RestClient restClient )
        throws ExitException
    {
        log.info( "Validating application ID {} with CLM server {}...", params.getApplicationId(),
                  params.getServerUrl() );
        ...
        catch ( Exception e )
        {
            if ( e instanceof HttpResponseException )
            {
                ...
                {
                    log.error( "The CLM server {} could not be contacted: {} ({})", params.getServerUrl(),
                               e.getMessage(), resp.getStatusCode() );
                }
            }
            else
            {
        ...
        throws ExitException
    {
        ...
        {
            log.error( "The policy evaluation results could not be fetched from the CLM server: {} ({})",
        ...
        {
            log.error( "The policy evaluation results could not be fetched from the CLM server: {} ({})",
                       e.getMessage(), e.getResult().status() );
            throw new ExitException( 6, e );
        }
        catch ( IOException e )
        {
        ...
        log.info( "Summary of policy violations: {} critical, {} severe, {} moderate",
                  eval.getCriticalComponentCount(), eval.getSevereComponentCount(), eval.getModerateComponentCount() );
    ...
    private ScanReceipt uploadScan( Parameters params, RestClient restClient, File scanFile )
        throws ExitException
    {
