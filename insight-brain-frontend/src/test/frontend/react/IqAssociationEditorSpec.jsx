...
describe('IqAssociationEditor', () => {
  ...
  it('renders error alert with default text', () => {
    ...
    expect(fieldSet).toHaveClassName('iq-association-editor--full-width');
  });
  ...
  it('renders 2 column for 10 or more items', () => {
    ...
    expect(fieldSet).toHaveClassName('iq-association-editor--multi-column');
  });
  ...
  it('renders a hexagon icon', () => {
    ...
    expect(icons[1]).toHaveClassName(`nx-selectable-color--${angularToRscColorMap[minimalProps.items[1].color]}`);
  });
  ...
  it('calls onChange after toggling item"s selectedParam', () => {
    ...
    expect(onChangeSpy).toHaveBeenCalledOnceWith({ ...minimalProps.items[1], isApplied: false });
  });
});
