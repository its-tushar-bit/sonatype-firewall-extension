...
describe('double.column.picker.directive.spec.js', function () {
  ...
  beforeEach(inject(function (_$compile_, $rootScope) {
    ...
    element = $compile(
      '<form><double-column-picker list="list" filter-placeholder="Filter" left-column-name="Left" ' +
        'right-column-name="Right" item-name-param="name"></double-column-picker></form>'
    )(scope).children();
  ...
  it('Directive moves 2 items from right to left', function () {
    scope.list = [
      { name: 'name1', picked: true, checked: true },
      { name: 'name2', picked: true, checked: true },
    ];
    scope.$apply();
    ...
    expect(angular.copy(vm.list)).toEqual([
      { name: 'name1', picked: false, checked: true },
      { name: 'name2', picked: false, checked: true },
    ]);
  });
  ...
  it('Directive checks all picked items and then unchecks all', function () {
    scope.list = [
      { name: 'name1', picked: true, checked: false },
      { name: 'name2', picked: true, checked: false },
    ];
    scope.$apply();
  ...
  it('Directive checks all un-picked items and then unchecks all', function () {
    scope.list = [
      { name: 'name1', picked: false, checked: false },
      { name: 'name2', picked: false, checked: false },
    ];
    scope.$apply();
