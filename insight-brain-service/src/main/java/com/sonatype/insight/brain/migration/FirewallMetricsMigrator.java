...
public class FirewallMetricsMigrator
{
  ...
  private void calculateNamespaceAttacksBlockedAndSupplyChainAttacksBlockedMetrics(List<Repository> repositories) {
    ...
    for (RepositoryPolicyViolationsMetrics repositoryPolicyViolationsMetrics : allMetrics) {
      if (CollectionUtils.isNotEmpty(repositoryPolicyViolationsMetrics.namespaceAttacksBlockedMetrics)) {
        allNamespaceAttacksBlockedMetrics.addAll(repositoryPolicyViolationsMetrics.namespaceAttacksBlockedMetrics);
      }
      if (CollectionUtils.isNotEmpty(repositoryPolicyViolationsMetrics.supplyChainAttacksBlockedMetrics)) {
  ...
  private RepositoryPolicyViolationsMetrics processRepositoryPolicyViolations(Repository repository, Date fromDate) {
    ...
    try (TransactionContext tx = repositoryPolicyViolationDAO.createTransactionContext()) {
      ...
      while (CollectionUtils.isNotEmpty(repositoryPolicyViolations));
    }
    catch (Exception e) {
  ...
  private boolean checkProductLicenseFeatures() {
    ...
    try {
      if (productLicense != null && StringUtils.isNotBlank(productLicense.getLicenseDetails())) {
        ...
        if (productLicenseDetailsDTO != null && CollectionUtils.isNotEmpty(productLicenseDetailsDTO.features)) {
          apiFirewallMetricsService.checkLicensedFeatures(
              productLicenseDetailsDTO.features.stream().map(LicensedFeature::valueOf).collect(toSet()));
          return true;
        }
      }
    }
  ...
  private void consolidateAndSaveFirewallMetrics(List<FirewallMetrics> allMetrics) {
    if (CollectionUtils.isNotEmpty(allMetrics)) {
