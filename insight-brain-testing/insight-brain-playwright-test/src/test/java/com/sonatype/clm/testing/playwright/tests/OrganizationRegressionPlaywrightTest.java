    ...
    extends AbstractIqUiTest
{
   ...
   * tile.
   * <p>
   * Explicitly enables all licensed features (including {@code custom-component-labels}) before
  ...
  public void testCreateComponentLabel_appearsInLabelsTile() {
    ...
    // Ensure custom-component-labels is present. AbstractIqUiTest @After resets automatically.
    setFeatures(EnumSet.allOf(LicensedFeature.class).toArray(new LicensedFeature[0]));

    navigateAndWaitForUrl(OwnerSummaryPage.url(org.getId()), OwnerSummaryPage.ORG_URL_FRAGMENT);
    // Reload flushes the Redux store so the SPA re-fetches features with custom-component-labels active.
    playwrightRefresh();
    assertions.shouldBeVisible();
   ...
   * <p>
   * {@code CUSTOM_APPLICATION_CATEGORIES} is included in the embedded test license by default;
