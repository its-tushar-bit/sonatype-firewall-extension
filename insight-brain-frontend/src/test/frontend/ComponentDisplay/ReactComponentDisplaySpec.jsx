...
describe('ComponentDisplay (React)', function() {
  ...
  it('renders an em child with "Unknown" if no displayName or filename is set', function() {
    ...
    expect(getShallowComponent({ component: { displayName: null, filename: null, filenames: null } }).find('em'))
        .toHaveText('Unknown');
    expect(getShallowComponent({ component: { displayName: null, filename: null, filenames: null } }).find('span'))
        .not.toExist();
  });
  ...
  it('renders an em child with text derived from the filenames if they are present', function() {
    ...
    expect(getShallowComponent({ component: componentWithFilenames }).find('em'))
        .toHaveText('foo.js, bar/baz.js');
    expect(getShallowComponent({ component: componentWithFilenames }).find('span'))
        .not.toExist();
  });
  ...
  it('renders an em child with text derived from the filename (singular) if it is present', function() {
    const componentWithFilename = { filename: 'asdf.js' },
        componentWithFilenameAndFilenames = { ...componentWithFilename, filenames: ['foo.js', 'bar/baz.js'] };
  ...
  it('renders an em child with text derived from a displayName consisting only of a Filename part', function() {
    const componentWithDisplayName = { displayName: { parts: [{ field: 'Filename', value: 'display.js' }] } },
        componentWithDisplayNameAndFilename = { ...componentWithDisplayName, filename: 'asdf.js' },
        componentWithDisplaynameAndFilenameAndFilenames = {
          ...componentWithDisplayNameAndFilename,
          filenames: ['foo.js', 'bar/baz.js']
        };

    [componentWithDisplayName, componentWithDisplayNameAndFilename, componentWithDisplaynameAndFilenameAndFilenames]
        .map(component => {
