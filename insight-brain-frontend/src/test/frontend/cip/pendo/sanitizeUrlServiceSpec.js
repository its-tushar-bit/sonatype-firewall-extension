...
describe('cip-loader sanitizeUrlService', function () {
  ...
  it('removes the baseUrl', function () {
    expect(sanitizeUrlService.sanitize('http://localhost:8070/assets/foo')).toBe('/assets/foo');

    baseUrl = 'https://foobar.com/iq';

    expect(sanitizeUrlService.sanitize('https://foobar.com/iq/assets/foo')).toBe('/assets/foo');
  });
