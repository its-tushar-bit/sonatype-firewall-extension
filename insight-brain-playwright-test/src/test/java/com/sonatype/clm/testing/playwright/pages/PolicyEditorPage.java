    ...
    extends BasePage
{
   ...
   * by clicking "Add a Policy". Combines network-idle (so the policy + constraint Redux slices
   * finish their initial loads and won't re-seed the form after a fill) with a visibility wait on
   * the first constraint-name input (the last element to render in the constraint slice).
   *
   * <p>
