...
/**
 * Converted from the legacy {@code VulnerabilitiesListResourceTest}. Kept in the original package because
 * {@link #seedComponentsReport} calls the package-private {@link VulnerabilitiesListTestSupport#populateIndex}.
 */
@IqH2Test
public class IqH2VulnerabilitiesListResourceTest
{
  ...
  public void listVulnerabilities_flagOn_returnsDistinctEstateRows() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnListTribe");
    Application app = ctx.tempEntity().newApplication("Vuln App", "vuln-app", org.getId());
    seedComponentsReport(app, "vulnListReport");
    User reader = readerOn(org, "vuln-list-reader");
    VulnerabilitiesListTestSupport.populateIndex(ctx.lookup(SearchIndexClient.class));
  ...
  public void listVulnerabilities_defaultSort_ordersByCvssDescending() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnSortTribe");
    Application app = ctx.tempEntity().newApplication("Sort Vuln App", "sort-vuln-app", org.getId());
  ...
  public void listVulnerabilities_sameCveAcrossApplications_countsOnce() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnEstateTribe");
    Application appOne = ctx.tempEntity().newApplication("Estate App One", "estate-app-1", org.getId());
    Application appTwo = ctx.tempEntity().newApplication("Estate App Two", "estate-app-2", org.getId());
  ...
  public void listVulnerabilities_catalogTab_usesCatalogSourceWithoutBlockingMyScanDataShape() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnCatalogTribe");
    Application app = ctx.tempEntity().newApplication("Catalog App", "catalog-app", org.getId());
  ...
  public void listVulnerabilities_failsClosed_userWithNoReadContexts() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnFailClosedTribe");
    Application app = ctx.tempEntity().newApplication("Fail Closed App", "fail-closed-app", org.getId());
    seedComponentsReport(app, "failClosedVulnReport");
    User noPerms = ctx.tempEntity().newUser("vuln-no-permissions");
    VulnerabilitiesListTestSupport.populateIndex(ctx.lookup(SearchIndexClient.class));
  ...
  public void listVulnerabilities_severityFilter_narrowsResultsAndKeepsSiblingFacets() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnSeverityTribe");
    Application app = ctx.tempEntity().newApplication("Severity Vuln App", "severity-vuln-app", org.getId());
  ...
  public void listVulnerabilities_catalogKevFilter_isAccepted() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnCatalogKevTribe");
    Application app = ctx.tempEntity().newApplication("Catalog Kev App", "catalog-kev-app", org.getId());
  ...
  public void listVulnerabilities_catalogEpssMinOnly_isAccepted() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnCatalogEpssTribe");
    Application app = ctx.tempEntity().newApplication("Catalog Epss App", "catalog-epss-app", org.getId());
  ...
  public void listVulnerabilities_cvssRangeFilter_narrowsResults() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnCvssTribe");
    Application app = ctx.tempEntity().newApplication("Cvss Vuln App", "cvss-vuln-app", org.getId());
  ...
  public void listVulnerabilities_ecosystemFilter_narrowsResults() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnEcosystemTribe");
    Application app = ctx.tempEntity().newApplication("Ecosystem Vuln App", "ecosystem-vuln-app", org.getId());
  ...
  public void listVulnerabilities_scopeFacets_countDistinctVulnerabilitiesPerOrgAppAndStage() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnScopeFacetTribe");
    Application app = ctx.tempEntity().newApplication("Scope Facet App", "scope-facet-app", org.getId());
  ...
  public void listVulnerabilities_organizationFilter_narrowsToThatOrgAndKeepsItsOwnFacet() throws Exception {
    ...
    Organization matching = ctx.tempEntity().newOrganization("VulnOrgFilterMatch");
    Organization other = ctx.tempEntity().newOrganization("VulnOrgFilterOther");
    Application matchingApp = ctx.tempEntity().newApplication("Org Filter App", "org-filter-app", matching.getId());
    Application otherApp =
        ctx.tempEntity().newApplication("Org Filter Other App", "org-filter-other-app", other.getId());
  ...
  public void listVulnerabilities_applicationFilter_narrowsResults() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnAppFilterTribe");
    Application matching = ctx.tempEntity().newApplication("App Filter Match", "app-filter-match", org.getId());
    Application other = ctx.tempEntity().newApplication("App Filter Other", "app-filter-other", org.getId());
  ...
  public void listVulnerabilities_stageFilter_narrowsResults() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnStageFilterTribe");
    Application app = ctx.tempEntity().newApplication("Stage Filter App", "stage-filter-app", org.getId());
  ...
  public void exportVulnerabilities_myScanData_returnsBomCsvWithBlastRadiusRows() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnExportTribe");
    Application appOne = ctx.tempEntity().newApplication("Export App One", "export-app-1", org.getId());
    Application appTwo = ctx.tempEntity().newApplication("Export App Two", "export-app-2", org.getId());
  ...
  public void exportVulnerabilities_failsClosed_userWithNoReadContexts() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnExportFailClosedTribe");
    Application app = ctx.tempEntity().newApplication("Export Fail Closed App", "export-fail-closed-app", org.getId());
    seedComponentsReport(app, "exportFailClosedVuln");
    User noPerms = ctx.tempEntity().newUser("vuln-export-no-permissions");
    VulnerabilitiesListTestSupport.populateIndex(ctx.lookup(SearchIndexClient.class));
  ...
  public void exportVulnerabilities_scopedUser_seesOnlyReadableApplications() throws Exception {
    ...
    Organization org = ctx.tempEntity().newOrganization("VulnExportScopedTribe");
    Application readable = ctx.tempEntity().newApplication("Export Readable App", "export-readable-app", org.getId());
    Application hidden = ctx.tempEntity().newApplication("Export Hidden App", "export-hidden-app", org.getId());
    seedComponentsReport(readable, "exportScopedReadable");
    seedComponentsReport(hidden, "exportScopedHidden");

    User reader = ctx.tempEntity().newUser("vuln-export-scoped-reader");
  ...
  private User readerOn(List<Organization> orgs, String username) {
    User reader = ctx.tempEntity().newUser(username);
    Role readRole = ctx.tempEntity().newRole(false /* global */, Permission.READ);
    for (Organization org : orgs) {
      ctx.tempEntity().newMembershipMapping(org.getId(), readRole.getId(), reader.getUsername());
    }
    return reader;
  }
