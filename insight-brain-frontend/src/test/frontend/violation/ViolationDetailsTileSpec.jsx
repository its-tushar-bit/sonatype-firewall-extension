...
describe('ViolationDetailsTile', function() {
  ...
  beforeEach(function() {
    ...
    ViolationDetailsTile = require('inject-loader!../../../main/frontend/violation/ViolationDetailsTile')({
      ...
      '../util/CLMContextLocation': { getOwnerImageUrl: getOwnerImageUrlMock }
    }).default;
  ...
  describe('header', function() {
    ...
    it('sets the correct threatLevelCategory on the ViolationExclamation', function() {
      ...
      }).find(ViolationExclamation)).toHaveProp('threatLevelCategory', 'severe');
      expect(getShallowComponent({
        violationDetails: {
          ...minimalProps.violationDetails,
          threatLevel: 3
        }
      }).find(ViolationExclamation)).toHaveProp('threatLevelCategory', 'moderate');
      expect(getShallowComponent({
        violationDetails: {
          ...minimalProps.violationDetails,
          threatLevel: 1
        }
      }).find(ViolationExclamation)).toHaveProp('threatLevelCategory', 'low');
      expect(getShallowComponent({
        violationDetails: {
          ...minimalProps.violationDetails,
          threatLevel: 0
  ...
  describe('threat level section', function() {
    ...
    it('sets the iq-threat-level class on the dd and a modifier class based on the threat level category',
        function() {
          const getDd = threatLevel => getShallowComponent({
            ...minimalProps,
            violationDetails: threatLevel == null ? minimalProps.violationDetails : {
              ...minimalProps.violationDetails,
              threatLevel
            }
  ...
  describe('policy owner section', function() {
    ...
    it('sets the iq-violation-details__policy-owner-icon class on the icon', function() {
      expect(getShallowComponent().find('.iq-violation-details__policy-owner img'))
    ...
    describe('when the policy owner is an org', function() {
      it('sets the link href using the management.view.organization state and the org id', function() {
        ...
        expect(stateHrefMock).toHaveBeenCalledWith('theState', { organizationId: '1234' });
      });
    ...
    describe('when the policy owner is an app', function() {
      const getComponentWithAppProps = () => getShallowComponent(pathSet(['violationDetails', 'policyOwner'], {
        ownerName: 'polOwner',
      ...
      it('sets the link href using the management.view.application state and the app public id', function() {
        ...
        expect(stateHrefMock).toHaveBeenCalledWith('theState', { applicationPublicId: 'app2' });
      });

      it('sets the icon URL using CLMContextLocation.getOwnerImageUrl', function() {
        expect(getComponentWithAppProps().find('.iq-violation-details__policy-owner img'))
            .toHaveProp('src', '/rest/icon');
