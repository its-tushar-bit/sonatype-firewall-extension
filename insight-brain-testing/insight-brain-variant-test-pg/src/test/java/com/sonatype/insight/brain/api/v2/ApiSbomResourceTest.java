...
public class ApiSbomResourceTest
{
  ...
  public void setUp() throws Exception {
    dao = ctx.lookup(ThirdPartySbomMetadataDAO.class);
  ...
  public void testGetSbomVersion_Current_CycloneDx() throws Exception {
    ...
    Path zippedBom = mockOriginalSbom(this.getClass(), "sboms/valid-cyclonedx-bom.xml",
        insightWork.getSbomDir(app.getId()).toPath());
    String sbomVersion = ctx.tempEntity().newRandomHash();
    setupScenarioWithMetadataComponentSecurityLicenseAndVex(ctx.tempEntity(), app, zippedBom, sbomVersion,
  ...
  public void testGetSbomMetadataSummaryForApplication_Successful() throws Exception {
    ...
    ThirdPartyFile file1 = ctx.tempEntity().newThirdPartyFile("CycloneDX -bom.xml");
    ThirdPartyFile file2 = ctx.tempEntity().newThirdPartyFile("SPDX .spdx.json");

    ThirdPartySbomMetadata sbom1 =
        ctx.tempEntity().newThirdPartySbomMetadata(file1.getId(), app.getId(), ACTIVE, file1.getFilename());
    ThirdPartySbomMetadata sbom2 =
        ctx.tempEntity().newThirdPartySbomMetadata(file2.getId(), app.getId(), ACTIVE, file2.getFilename());

    ThirdPartyFileCoordinate c1 = ctx.tempEntity().newThirdPartyFileCoordinate(file1, "s1", "f1", "n1", "v1");
    ThirdPartyFileCoordinate c2 = ctx.tempEntity().newThirdPartyFileCoordinate(file2, "s2", "f2", "n2", "v2");

    ThirdPartyCoordinateSecurity cs1 =
        ctx.tempEntity().newThirdPartyCoordinateSecurity(c1, "r1", sbom1.getId(), "d1", "l1", 3.5F, "sd1", "f1");
    ctx.tempEntity().newThirdPartyCoordinateSecurity(c1, "r2", sbom1.getId(), "d2", "l2", 7.5F, "sd2", "f2");
    ctx.tempEntity().newThirdPartyCoordinateSecurity(c2, "r3", sbom2.getId(), "d3", "l3", 1.5F, "sd3", "f3");
    ctx.tempEntity().newThirdPartyCoordinateSecurity(c2, "r4", sbom2.getId(), "d4", "l4", 0.5F, "sd4", "f4");

    ctx.tempEntity()
        .newThirdPartyVulnerabilityExploitabilityExchange(cs1, cs1.getRefId(),
  ...
  public void testGetSbomComponents_Successful() throws Exception {
    ...
    ThirdPartyScan scan = ctx.tempEntity().newThirdPartyScan(thirdPartyFile);
    ThirdPartySbomMetadata sbomMetadata = SbomMetadataBuilder.newSbomMetadataBuilder(ctx.daoFactory())
        .withApplicationId(app.getId())
    ...
    ThirdPartyFileCoordinate coordinate = ctx.tempEntity()
        .newThirdPartyFileCoordinate("86163fcc32524261bfd2bdbedb7eae42",
    ...
    ctx.tempEntity()
        .newThirdPartyCoordinateSecurity(coordinate, "refId", "description", "link",
    ...
    FileUtils.copyURLToFile(ReportHelper
        .zipReport("/ApiSbomServicePolicyViolationsTest", ctx.tempFolder()), reportFile);

    ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, scan.getScanId());
  ...
  public void testGetSbomComponents_SuccessfulWithDefaultValues() throws Exception {
    ...
    ThirdPartySbomMetadata sbomMetadata =
        SbomMetadataBuilder.newSbomMetadataBuilder(ctx.daoFactory())
            .withApplicationId(app.getId())
            .withThirdPartyFileId(thirdPartyFile.getId())
            .build();
    ...
    for (int i = 0; i < totalCountOfComponents; i++) {
      ...
      ThirdPartyFileCoordinate coordinate = ctx.tempEntity()
          .newThirdPartyFileCoordinate(sbomMetadata.getThirdPartyFileId(),
              "source", packageUrlIdentifier.getFormat(), packageUrlIdentifier.getName(),
              packageUrlIdentifier.getVersion(),
      ...
      ctx.tempEntity()
          .newThirdPartyCoordinateSecurity(coordinate, "refId" + i, "description" + i, "link" + i,
  ...
  public void testGetSbomComponentsByThirdPartyFileId_ComponentNameFilter() throws Exception {
    ...
    ThirdPartySbomMetadata sbomMetadata =
        ctx.tempEntity().newThirdPartySbomMetadata(thirdPartyFile.getId(), application.getId(), ACTIVE, "bom.xml");
    ...
    ctx.tempEntity()
        .newThirdPartyFileCoordinate(sbomMetadata.getThirdPartyFileId(),
            "s3", packageUrlIdentifier3.getFormat(), packageUrlIdentifier3.getName(),
            packageUrlIdentifier3.getVersion(),
    ...
    FileUtils.copyURLToFile(ReportHelper.zipReport("/ApiSbomServicePolicyViolationsTest", ctx.tempFolder()),
        reportFile);

    ctx.tempEntity().newPolicyEvaluation(application.getId(), BuildStageType.ID, thirdPartyScan.getScanId());
  ...
  public void testGetSbomComponentsByThirdPartyFileId_LicenseNameFilter() throws Exception {
    ...
    ThirdPartyFileCoordinate coordinate4 = ctx.tempEntity()
        .newThirdPartyFileCoordinate(sbomMetadata.getThirdPartyFileId(),
            "s4", packageUrlIdentifier4.getFormat(), packageUrlIdentifier4.getName(),
            packageUrlIdentifier4.getVersion(),
            "h4", packageUrlIdentifier4.getPackageUrl(), UNSPECIFIED);

    ctx.tempEntity().newThirdPartyCoordinateLicense(coordinate1, "license-1", "License 1", "http://license1");
    ctx.tempEntity().newThirdPartyCoordinateLicense(coordinate2, "license-1", "License 1", "http://license1");
    ctx.tempEntity().newThirdPartyCoordinateLicense(coordinate2, "license-3", "SpecialChars %$3", "http://license3");
    ctx.tempEntity().newThirdPartyCoordinateLicense(coordinate2, "license-4", "Another 4", "http://license4");
    ctx.tempEntity().newThirdPartyCoordinateLicense(coordinate4, "some-5", "Some 5", "http://some5");
  ...
  public void testGetSbomComponents_withLicenseOverrides() throws Exception {
    ...
    ctx.tempEntity()
        .newLicenseOverride(application.getId(), componentIdentifier2, LicenseOverrideStatus.SELECTED,
            Set.of("Aladdin", "MIT"));
  ...
  public void testImportSbom_Binary() throws Exception {
    ...
    File binaryFile = ctx.tempFolder().newFile("binary-scan");
  ...
  public void testImportSbom_EmptyApplicationId() throws Exception {
    ...
    ctx.assertResponseStatus(Status.BAD_REQUEST.getStatusCode(), response);
  ...
  public void testSaveVulnerabilityAnalysis() throws Exception {
    ...
    ThirdPartySbomMetadata sbomMetadata =
        ctx.tempEntity().newThirdPartySbomMetadata(thirdPartyFile.getId(), app.getId(), ACTIVE, "file.tgz");
    ThirdPartyFileCoordinate component =
        ctx.tempEntity()
            .newThirdPartyFileCoordinate(thirdPartyFile, "ThirdParty", "npm", "bloom", "1.0", "hash001",
                "pkg:npm/bloom@1.0");
    ctx.tempEntity().newThirdPartyCoordinateSecurity(component, refId, "description", "link", 8.1, "Critical", "1.2.0");
  ...
  public void testDeleteVulnerabilityAnalysis() throws Exception {
    ...
    ThirdPartyCoordinateSecurity security =
        ctx.tempEntity()
            .newThirdPartyCoordinateSecurity(component, refId, "description", "link", 8.1, "Critical", "1.2.0");
    ctx.tempEntity()
    ...
    ctx.assertResponseStatus(Status.NO_CONTENT.getStatusCode(), response);
  }
  ...
  private void assertSbomMetadataIdIsSetOnThirdPartyCoordinateSecurityEntities(ApiSbomStatusDTO resultDTO) {
    ThirdPartySbomMetadata thirdPartySbomMetadata = ctx.lookup(ThirdPartySbomMetadataDAO.class)
        .getByApplicationIdAndSbomVersion(resultDTO.applicationId, resultDTO.version);
    assertThat(thirdPartySbomMetadata).isNotNull();
    List<ThirdPartyFileCoordinate> thirdPartyFileCoordinates = ctx.lookup(ThirdPartyFileCoordinateDAO.class)
    ...
    if (CollectionUtils.isNotEmpty(thirdPartyFileCoordinateIds)) {
      List<ThirdPartyCoordinateSecurity> thirdPartyCoordinateSecurities =
          ctx.lookup(ThirdPartyCoordinateSecurityDAO.class)
