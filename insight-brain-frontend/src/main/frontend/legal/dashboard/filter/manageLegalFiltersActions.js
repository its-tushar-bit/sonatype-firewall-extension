...
export function fetchSavedFilters() {
  return dispatch => {
    return axios.get(getLegalDashboardSavedFilters())
...
export function saveFilter({name, isOverwriting}) {
  return (dispatch, getState) => {
    ...
    if (!saveFilterWarning) {
      if (isOverwriting) {
        return dispatch({ type: LEGAL_DASHBOARD_SAVE_FILTER_OVERWRITE_REQUESTED });
      }
    ...
    return axios.put(getLegalDashboardSavedFilters(), namedFilter)
        .catch(error => {
          dispatch({ type: LEGAL_DASHBOARD_SAVE_FILTER_FAILED, payload: error });
          return Promise.reject(error);
        })
        .then(({ data }) => {
          dispatch({ type: LEGAL_DASHBOARD_SAVE_FILTER_FULFILLED, payload: data });
          setTimeout(() => {
            dispatch({ type: LEGAL_DASHBOARD_SET_DISPLAY_SAVE_FILTER_MODAL, payload: false });
...
export function deleteFilter(filterName) {
  return dispatch => {
    ...
    return axios.delete(getLegalDashboardDeleteFilterUrl(filterName))
        .catch(error => {
          dispatch({ type: LEGAL_DASHBOARD_DELETE_FILTER_FAILED, payload: Messages.getHttpErrorMessage(error) });
          return Promise.reject(error);
        })
        .then(() => {
          dispatch({ type: LEGAL_DASHBOARD_DELETE_FILTER_FULFILLED, payload: filterName });
          setTimeout(() => {
