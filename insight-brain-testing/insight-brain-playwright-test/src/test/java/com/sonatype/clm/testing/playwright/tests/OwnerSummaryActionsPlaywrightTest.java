    ...
    extends AbstractIqUiTest
{
   ...
   * Installs an SBOM Manager license at runtime via {@code setLicensedProducts}; reverted by
   ...
   * modal. Explicitly enables all licensed features (including {@code custom-policies}) before
   * navigating so the modal always renders in enterprise mode; {@code AbstractIqUiTest}'s
  ...
  public void testImportPolicies_uploadAndConfirm() throws Exception {
    ...
    // Ensure custom-policies is present — mirror of setMissingFeature() in
    // testImportPolicies_previewModeOnNonEnterprise. AbstractIqUiTest @After resets automatically.
    setFeatures(EnumSet.allOf(LicensedFeature.class).toArray(new LicensedFeature[0]));

    navigateAndWaitForUrl(OwnerSummaryPage.url(org.getId()), OwnerSummaryPage.ORG_URL_FRAGMENT);
    // Reload flushes the Redux store so the SPA re-fetches features with custom-policies active.
    playwrightRefresh();
    assertions.shouldBeVisible();
