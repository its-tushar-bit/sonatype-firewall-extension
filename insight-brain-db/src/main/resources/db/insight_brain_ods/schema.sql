...
-- CLM-39870: Synchronous hosted-repository policy-enforcement blocks.
-- Each block attempt gets its own row; no unique constraint on (repository_id, pathname) so
-- developer retries preserve history. Rows are garbage-collected by a periodic cleanup task
-- based on the configured retention TTL. Continuous monitoring never reads from these tables.
CREATE TABLE hosted_deployment_block (
  ...
  CONSTRAINT hosted_deployment_block_repository_fk FOREIGN KEY (repository_id) REFERENCES repository(repository_id) ON DELETE CASCADE
);
CREATE INDEX hosted_deployment_block_repository_time_idx ON hosted_deployment_block(repository_id, blocked_time);
CREATE INDEX hosted_deployment_block_cleanup_idx ON hosted_deployment_block(blocked_time);

CREATE TABLE hosted_deployment_block_violation (
