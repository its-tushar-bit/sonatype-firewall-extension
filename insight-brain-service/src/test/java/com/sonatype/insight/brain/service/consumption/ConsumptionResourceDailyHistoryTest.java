...
public class ConsumptionResourceDailyHistoryTest
    extends AbstractConsumptionResourceTest
{
  private static final String DAILY_HISTORY_PATH = "daily-history";
  ...
  @Test
  public void getDailyHistory_asAdmin_returns200() throws Exception {
    User adminUser = createSystemAdminUser();

    HttpResponse response = restRequest().auth(adminUser).path(DAILY_HISTORY_PATH).get();
  ...
  @Test
  public void getDailyHistory_asUsageViewer_returns200() throws Exception {
    User usageViewerUser = createUsageViewerUser();

    HttpResponse response = restRequest().auth(usageViewerUser).path(DAILY_HISTORY_PATH).get();

    assertResponseStatus(HttpStatus.SC_OK, response);
  }

  @Test
  public void getDailyHistory_asUnauthorized_returns403() throws Exception {
    User regularUser = tempEntity.newUser();

    HttpResponse response = restRequest().auth(regularUser).path(DAILY_HISTORY_PATH).get();

    assertResponseStatus(HttpStatus.SC_FORBIDDEN, response);
  }

  @Test
  public void getDailyHistory_unauthenticated_returns401() throws Exception {
    HttpResponse response = restRequest().anon().path(DAILY_HISTORY_PATH).get();
