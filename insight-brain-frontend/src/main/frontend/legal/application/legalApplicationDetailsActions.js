...
const legalApplicationDetailsLoadAppFulfilled =
  payloadParamActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_APP_FULFILLED);
const legalApplicationDetailsLoadAppFailed = payloadParamActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_APP_FAILED);

const legalApplicationDetailsLoadStageRequested =
  noPayloadActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_STAGE_REQUESTED);
const legalApplicationDetailsLoadStageFulfilled =
  payloadParamActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_STAGE_FULFILLED);
const legalApplicationDetailsLoadStageFailed = payloadParamActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_STAGE_FAILED);

const legalApplicationDetailsLoadComponentsRequested =
  noPayloadActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_COMPONENTS_REQUESTED);
const legalApplicationDetailsLoadComponentsFulfilled =
  payloadParamActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_COMPONENTS_FULFILLED);
const legalApplicationDetailsLoadComponentsFailed =
  payloadParamActionCreator(LEGAL_APPLICATION_DETAILS_LOAD_COMPONENTS_FAILED);

export function loadApplication(applicationPublicId, stageTypeId) {
  return (dispatch) => {
    dispatch(legalApplicationDetailsLoadAppRequested());

    return axios.get(getApplicationUrl(applicationPublicId))
        .then(response => {
...
function loadStageType(stageTypeId) {
  return (dispatch) => {
    ...
    return axios.get(getActionStageUrl())
        .then(response => {
...
function loadComponents(applicationPublicId, stageTypeId) {
  return (dispatch) => {
    ...
    return axios.post(getLegalDashboardApplicationUrl(applicationPublicId), { stageTypeIds: [stageTypeId] })
