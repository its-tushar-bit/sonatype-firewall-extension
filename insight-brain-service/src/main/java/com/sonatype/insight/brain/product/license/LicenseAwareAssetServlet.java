...
/**
 * This is effectively a carbon copy of AssetServlet, with the ability to load index file from different locations
 */
public class LicenseAwareAssetServlet
    extends HttpServlet
{
    private static final long serialVersionUID = 7821458602127531773L;
    ...
    private final String licensedResourcePath;

    private final String unlicensedResourcePath;

    private final String uriPath;

    private final String indexFile;

    public LicenseAwareAssetServlet( String licensedResourcePath, String unlicensedResourcePath, String uriPath,
                                     String indexFile )
    {
        final String trimmedLicensedPath = CharMatcher.is( '/' ).trimFrom( licensedResourcePath );
        this.licensedResourcePath = trimmedLicensedPath.isEmpty() ? trimmedLicensedPath : trimmedLicensedPath + "/";
        final String trimmedUnlicensedPath = CharMatcher.is( '/' ).trimFrom( unlicensedResourcePath );
        this.unlicensedResourcePath =
            trimmedUnlicensedPath.isEmpty() ? trimmedUnlicensedPath : trimmedUnlicensedPath + "/";
        ...
        throws ServletException, IOException
    {
        try
        {
            ...
            if ( !req.getRequestURI().contains( getIndexFile() ) && isCachedClientSide( req, cachedAsset ) )
            {
    ...
    private CachedAsset loadAsset( String key )
        throws URISyntaxException, IOException
    {
        ...
        final String absoluteRequestedResourcePath =
            CharMatcher.is( '/' ).trimFrom( getResourcePath( requestedResourcePath ) + requestedResourcePath );
    ...
    {
        return ( requestPath.contains( getIndexFile() ) && !isLicensed() ) ? unlicensedResourcePath
