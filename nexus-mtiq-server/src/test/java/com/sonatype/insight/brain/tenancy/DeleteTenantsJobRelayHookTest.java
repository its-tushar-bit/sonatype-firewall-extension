...
public class DeleteTenantsJobRelayHookTest
{
  ...
  public void before() {
    ...
    lenient().when(tenantValidator.validateTenantExists(anyString())).thenReturn(true);
    // Stub heavy substeps so the test is hermetic.
    lenient().doReturn(true).when(job).deleteAuth0Resources(any());
    lenient().doReturn(true).when(job).deleteJobs(any());
    lenient().doReturn(true).when(job).deleteDatabaseSchema(any());
    lenient().doReturn(true).when(job).deleteFilesOnDisk(any());
  }
