...
export default function AdvancedSearchResultCard({searchResultItem, groupIdentifier, $state}) {
  ...
  const getOrgHref = () => $state.href($state.get('management.view.organization'), {
        organizationId: searchResultItem.organizationId
      }),
      getAppHref = () => $state.href($state.get('management.view.application'), {
        applicationPublicId: searchResultItem.applicationPublicId
      }),
      getCategoryHref = () => $state.href($state.get('management.edit.organization.category'), {
        organizationId: searchResultItem.organizationId,
        categoryId: searchResultItem.applicationCategoryId
      }),
      getOrgLabelHref = () => $state.href($state.get('management.edit.organization.label'), {
        organizationId: searchResultItem.organizationId,
        labelId: searchResultItem.componentLabelId
      }),
      getAppLabelHref = () => $state.href($state.get('management.edit.application.label'), {
        applicationPublicId: searchResultItem.applicationPublicId,
        labelId: searchResultItem.componentLabelId
      }),
      getReportHref = () => $state.href($state.get('applicationReport.policy'), {
        publicId: searchResultItem.applicationPublicId, scanId: searchResultItem.reportId
      }),
      getVulnHref = () => $state.href($state.get('vulnerabilitySearchDetail'), {id: searchResultItem.vulnerabilityId}),
      getOrgPolicyHref = () => $state.href($state.get('management.edit.organization.policy'), {
        organizationId: searchResultItem.organizationId,
        policyId: searchResultItem.policyId
      }),
      getAppPolicyHref = () => $state.href($state.get('management.edit.application.policy'), {
        applicationPublicId: searchResultItem.applicationPublicId,
        policyId: searchResultItem.policyId
      });
          ...
          <tr className="nx-table-row">
            ...
            <td className="nx-cell">
              { searchResultItem.organizationId &&
                <a href={getOrgLabelHref()}>{searchResultItem.componentLabelName}</a>
              }
