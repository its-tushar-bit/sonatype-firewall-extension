...
import { getSecurityEventAffectedComponents, AFFECTED_COMPONENTS_SORT_FIELDS } from 'GuideRoot/api/securityEventsBackend';
...
export function SecurityEventComponentsImpactedTab() {
  ...
  useEffect(() => {
    ...
    getSecurityEventAffectedComponents(eventId, { query, offset, limit, sortField, sortOrder })
      .then((data) => {
        if (!cancelled) {
  ...
  // Component is only rendered under /security-event/:eventId — this guard prevents a broken formAction
  ...
  return (
    <Flex direction="column" gap={tokens.space.section}>
      <AffectedComponentsTable
        ...
        formAction={`/security-event/${eventId}/affected-components`}
