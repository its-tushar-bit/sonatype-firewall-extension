...
describe('audit-report sanitizeUrlService', function() {
  ...
  beforeEach(angular.mock.module(auditReportPendoModule.name, function($provide) {
  ...
  it('removes the baseUrl', function() {
    expect(sanitizeUrlService.sanitize('http://localhost:8070/assets/audit-report/index.html'))
        .toBe('/assets/audit-report/index.html');

    baseUrl = 'https://foobar.com/iq';

    expect(sanitizeUrlService.sanitize('https://foobar.com/iq/assets/audit-report/index.html'))
        .toBe('/assets/audit-report/index.html');
  });
