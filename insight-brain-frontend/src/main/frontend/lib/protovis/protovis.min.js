...
try {
  ...
  pv.parse = function (b) {
    for (
      var c = new RegExp('function\\s*(\\b\\w+)?\\s*\\([^)]*\\)\\s*', 'mg'),
        d,
        f,
        g = 0,
        h = '';
...
pv.css = function (b, c) {
  return window.getComputedStyle
    ? window.getComputedStyle(b, null).getPropertyValue(c)
    : b.currentStyle[c];
};
pv.error = function (b) {
...
pv.Format.number = function () {
  ...
  b.parse = function (r) {
    var s = pv.Format.re;
    r = String(r)
      .replace(new RegExp('^(' + s(j) + ')*'), '')
      .replace(new RegExp('(' + s(k) + ')*$'), '')
      .split(q);
...
pv.range = function (b, c, d) {
  ...
  var f = [],
    ...
    h;
  c -= (c - b) * 1.0e-10;
  if (d < 0) for (; (h = b + d * g++) > c; ) f.push(h);
  else for (; (h = b + d * g++) < c; ) f.push(h);
  return f;
};
pv.random = function (b, c, d) {
  ...
  if (d == undefined) d = 1;
  return d
    ? Math.floor((Math.random() * (c - b)) / d) * d + b
    : Math.random() * (c - b) + b;
};
pv.sum = function (b, c) {
  var d = {};
...
pv.logFloor = function (b, c) {
  return b > 0
    ? Math.pow(c, Math.floor(pv.log(b, c)))
    : -Math.pow(c, -Math.floor(-pv.log(-b, c)));
};
pv.logCeil = function (b, c) {
  return b > 0
    ? Math.pow(c, Math.ceil(pv.log(b, c)))
    : -Math.pow(c, -Math.ceil(-pv.log(-b, c)));
};
(function () {
  var b = Math.PI / 180,
...
pv.Tree.prototype.map = function () {
  for (var b = {}, c = {}, d = 0; d < this.array.length; d++) {
    c.index = d;
    for (
      var f = this.array[d], g = this.k.call(c, f), h = b, i = 0;
      i < g.length - 1;
      i++
    )
      h = h[g[i]] || (h[g[i]] = {});
...
pv.Flatten.prototype.array = function () {
  ...
  if (g) {
    function h(i, j) {
      if (g(i)) c.push({ keys: d.slice(), value: i });
      else
        for (var k in i) {
          d.push(k);
          h(i[k], j + 1);
...
a.plus = function (b, c) {
  return arguments.length == 1
    ? new pv.Vector(this.x + b.x, this.y + b.y)
    : new pv.Vector(this.x + b, this.y + c);
};
...
pv.Scale.interpolator = function (b, c) {
  ...
  return function (d) {
    ...
    if (f < 1.0e-5) f = 0;
    return b.a == 0
      ? pv.rgb(c.r, c.g, c.b, f)
      : c.a == 0
      ? pv.rgb(b.r, b.g, b.b, f)
      : pv.rgb(
          Math.round(b.r * (1 - d) + c.r * d),
          Math.round(b.g * (1 - d) + c.g * d),
          Math.round(b.b * (1 - d) + c.b * d),
          f
        );
  };
};
pv.Scale.quantitative = function () {
  ...
  c.domain = function (n, p, m) {
    if (arguments.length) {
      ...
      if (n instanceof Array) {
        if (arguments.length < 2) p = pv.identity;
  ...
  c.range = function () {
    if (arguments.length) {
      ...
      for (var n = 0; n < g.length - 1; n++)
        h.push(pv.Scale.interpolator(g[n], g[n + 1]));
      return this;
    }
    return g;
  };
  ...
  c.ticks = function (n) {
    ...
    p = Math.ceil(s / v) * v;
    m = Math.floor(m / v) * v;
    q = pv.Format.number().fractionDigits(
      Math.max(0, -Math.floor(pv.log(v, 10) + 0.01))
    );
    m = pv.range(p, m + v, v);
...
a.brighter = function (b) {
  ...
  if (f && f < 30) f = 30;
  return pv.rgb(
    Math.min(255, Math.floor(c / b)),
    Math.min(255, Math.floor(d / b)),
    Math.min(255, Math.floor(f / b)),
    this.a
  );
};
a.darker = function (b) {
  b = Math.pow(0.7, arguments.length ? b : 1);
  return pv.rgb(
    Math.max(0, Math.floor(b * this.r)),
    Math.max(0, Math.floor(b * this.g)),
    Math.max(0, Math.floor(b * this.b)),
    this.a
  );
};
...
pv.SvgScene.curveHermite = function (b, c) {
  ...
  if (c.length > 1) {
    ...
    for (g = 2; g < c.length; g++, k++) {
      ...
      j = c[g];
      f +=
        'S' + (h.left - j.x) + ',' + (h.top - j.y) + ',' + h.left + ',' + h.top;
    }
  }
  if (d) {
    b = b[k];
...
pv.SvgScene.area = function (b) {
  ...
  function d(n, p) {
    ...
    return (
      m[0].left + ',' + m[0].top + n + 'L' + r[0].left + ',' + r[0].top + p
    );
  }
  ...
  for (var j = [], k, l = 0; l < b.length; l++) {
    ...
    if (k.width || k.height) {
      ...
      j.push(
        (q - l > 2 &&
          (g.interpolate == 'basis' ||
            g.interpolate == 'cardinal' ||
            g.interpolate == 'monotone')
          ? d
          : c)(l, q - 1)
      );
      l = q - 1;
...
pv.SvgScene.areaSegment = function (b) {
  ...
  if (
    d.interpolate == 'basis' ||
    d.interpolate == 'cardinal' ||
    d.interpolate == 'monotone'
  ) {
    f = [];
    g = [];
  ...
  for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
    ...
    if (f.visible) {
      ...
      if (f.image) {
        ...
        var g =
          c.firstChild ||
          c.appendChild(document.createElementNS(this.xhtml, 'canvas'));
        ...
        c = this.expect(
          ...
          'text',
          {
            ...
            transform:
              ...
              ')' +
              (f.textAngle
                ? ' rotate(' + (180 * f.textAngle) / Math.PI + ')'
                : '') +
    ...
    if (h.visible && i.visible) {
      ...
      if (j.opacity) {
        if (h.interpolate == 'linear' && h.lineJoin == 'miter') {
          ...
          i = f ? f[d] : 'M' + h.left + ',' + h.top + this.pathSegment(h, i);
        c = this.expect(c, 'path', {
      ...
      if (!b.parent) {
        ...
        if (!c) {
          ...
          for (var h = 0; h < this.events.length; h++)
            c.addEventListener(this.events[h], this.dispatch, false);
          d = c.firstChild;
        }
        b.$g = c;
...
a.cousin = function () {
  var b = this.parent;
  return (b = b && b.sibling()) && b.children
    ? b.children[this.childIndex][this.index]
    : null;
};
a.render = function () {
  function b(i, j, k) {
...
a.bind = function () {
  ...
  while ((h = h.proto));
  h = d[0].concat(d[1]);
  for (i = 0; i < h.length; i++) this.propertyMethod(h[i].name, true);
  this.binds = {
    properties: c,
    data: f,
    defs: h,
...
pv.Area.prototype.anchor = function (b) {
  return pv.Mark.prototype.anchor
    .call(this, b)
...
a.anchor = function (b) {
  return pv.Area.prototype.anchor
    .call(this, b)
    .textAlign(function () {
      switch (this.name()) {
        case 'left':
    ...
    if (c) {
      ...
      if (!(c = d[this.index])) {
        ...
        if (this.$dom) this.$dom.parentNode.insertBefore(c, this.$dom);
          ...
          for (d = document.body; d.lastChild && d.lastChild.tagName; )
            d = d.lastChild;
  ...
  if (b.visible) {
    ...
    if (b.url == null && this.binds.image) {
      ...
      for (d = c = 0; c < g; c++) {
        ...
        for (var i = 0; i < f; i++) {
          h[0] = i;
          var j = this.binds.image.apply(this, h);
          b[d++] = j.r;
          b[d++] = j.g;
          b[d++] = j.b;
          b[d++] = 255 * j.a;
  ...
  return pv.Mark.prototype.anchor
    ...
    .textAlign(function () {
      ...
      if (c(g))
        switch (this.name()) {
          case 'outer':
            return pv.Wedge.upright(f(g)) ? 'right' : 'left';
          case 'inner':
...
pv.Force.charge = function (b) {
  ...
  k.apply = function (l, q) {
    c(q.root);
    for (l = l; l; l = l.next) d(q.root, l, q.xMin, q.yMin, q.xMax, q.yMax);
  };
  return k;
};
...
pv.Force.spring = function (b) {
  ...
  h.links = function (i) {
    if (arguments.length) {
      ...
      g = i.map(function (j) {
        return (
          1 /
          Math.sqrt(Math.max(j.sourceNode.linkDegree, j.targetNode.linkDegree))
        );
      });
      return h;
    }
  ...
  h.apply = function () {
    for (var i = 0; i < f.length; i++) {
      ...
      p =
        (b * g[i] * (n - d) +
          c * g[i] * (l * (j.vx - k.vx) + q * (j.vy - k.vy)) * p) *
        p;
...
pv.Layout.Network.prototype = pv
  .extend(pv.Layout)
  .property('nodes', function (b) {
  ...
  buildImplied: function (b) {
    ...
    function g(o) {
      switch (n) {
        ...
        case 'top':
        case 'bottom':
          return (o.maxBreadth - o.minBreadth) * m;
        case 'radial':
          return o.parentNode
            ? (o.innerRadius + o.outerRadius) * Math.cos(o.midAngle)
            : 0;
      }
    }
    function h(o) {
      switch (n) {
        ...
        case 'radial':
          return o.parentNode
            ? (o.innerRadius + o.outerRadius) * Math.sin(o.midAngle)
            : 0;
      }
    }
    function i(o) {
    ...
    function k(o) {
      return (o.parentNode ? o.minBreadth - 0.25 : 0) * 2 * Math.PI;
    }
    function l(o) {
...
pv.Layout.Stack = function () {
  ...
  this.buildImplied = function (i) {
    ...
    for (var u = 0; u < k; u++) {
      ...
      x.parent.index = u;
      t[0] = j[u];
      g[u] = this.$values.apply(x.parent, t);
      if (!u) l = g[u].length;
      t.unshift(null);
      for (var o = 0; o < l; o++) {
        t[0] = g[u][o];
...
a.defaults = new pv.Layout.Stack()
  .extend(pv.Layout.prototype.defaults)
  .orient('bottom-left')
  .offset('zero')
  .layers([[]]);
...
pv.Layout.Treemap.prototype = pv
  .extend(pv.Layout.Hierarchy)
...
a.defaults = new pv.Layout.Treemap()
...
a.buildImplied = function (b) {
  ...
  function f(r, s) {
    ...
    if (m != 'squarify')
      ...
      if (!(r.size <= 0)) {
        ...
        if (t(w)) for (s = 0; s < w.length; s++) w[s].dy += v;
        else for (s = 0; s < w.length; s++) w[s].dx += o;
      }
    }
  }
  if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
    ...
    h.visitAfter(function (r, s) {
      ...
      r.x = r.y = r.dx = r.dy = 0;
      r.size = r.firstChild
        ? pv.sum(r.childNodes, function (t) {
            return t.size;
          })
...
pv.Layout.Force.prototype = pv
  .extend(pv.Layout.Network)
...
pv.Layout.Force.prototype.buildImplied = function (b) {
  ...
  if (pv.Layout.Network.prototype.buildImplied.call(this, b)) {
    ...
    l.force(
      pv.Force.charge(b.chargeConstant)
        .domain(b.chargeMinDistance, b.chargeMaxDistance)
        .theta(b.chargeTheta)
    );
    l.force(
      pv.Force.spring(b.springConstant)
        .damping(b.springDamping)
        .length(b.springLength)
        .links(g)
    );
...
pv.Layout.Cluster = function () {
  ...
  this.buildImplied = function (d) {
    c.call(this, d);
    b = /^(top|bottom)$/.test(d.orient)
      ? 'step-before'
      : /^(left|right)$/.test(d.orient)
      ? 'step-after'
      : 'linear';
  };
...
pv.Layout.Cluster.prototype = pv
  .extend(pv.Layout.Hierarchy)
  .property('group', Number)
  .property('orient', String)
  .property('innerRadius', Number)
  ...
  if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
    ...
    c.visitAfter(function (k) {
      if (k.firstChild)
        k.depth =
          1 +
          pv.max(k.childNodes, function (l) {
            return l.depth;
          });
      else {
        if (d && j != k.parentNode) {
    ...
    c.visitAfter(function (k) {
      if (k.firstChild)
        k.breadth = pv.mean(k.childNodes, function (l) {
          return l.breadth;
        });
      else {
        if (d && j != k.parentNode) {
...
pv.Layout.Cluster.Fill.prototype.buildImplied = function (b) {
  pv.Layout.Cluster.prototype.buildImplied.call(this, b) ||
    pv.Layout.Hierarchy.Fill.buildImplied.call(this, b);
};
pv.Layout.Partition = function () {
  pv.Layout.Hierarchy.call(this);
};
pv.Layout.Partition.prototype = pv
  .extend(pv.Layout.Hierarchy)
  .property('order', String)
  .property('orient', String)
  .property('innerRadius', Number)
...
pv.Layout.Partition.prototype.buildImplied = function (b) {
  if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
    ...
    d.visitAfter(function (i, j) {
      if (j > g) g = j;
      i.size = i.firstChild
        ? pv.sum(i.childNodes, function (k) {
            return k.size;
          })
    ...
    d.visitBefore(function (i) {
      for (
        var j = i.minBreadth, k = i.maxBreadth - j, l = i.firstChild;
        l;
        l = l.nextSibling
      ) {
        l.minBreadth = j;
...
pv.Layout.Partition.Fill.prototype.buildImplied = function (b) {
  pv.Layout.Partition.prototype.buildImplied.call(this, b) ||
    pv.Layout.Hierarchy.Fill.buildImplied.call(this, b);
};
...
pv.Layout.Horizon.prototype = pv
  .extend(pv.Layout)
    ...
    for (m = 0; m < f.length; m++) {
      ...
      h =
        !g && d.index > k.index
          ...
          : d.index + ',' + k.index;
      (n = q[h]) ||
        (n = q[h] = { sourceNode: d, targetNode: k, linkValue: 0, links: [] });
      n.links.push(f[m]);
...
pv.Layout.Bullet = function () {
  ...
  this.buildImplied = function (k) {
    ...
    i = pv
      .ramp('steelblue', 'lightsteelblue')
      .domain(0, Math.max(1, j.measures.length - 1));
  };
  ...
  (this.measure = new pv.Mark())
    ...
    .bottom(function () {
      return f == 'bottom' ? 0 : g ? this.parent.height() / 3.25 : null;
    })
    .fillStyle(function () {
      return i(this.index);
    }).parent = b;
...
pv.Behavior.pan = function () {
  ...
  function c() {
    if (f) {
      f.mark.context(f, g, function () {
        var l = h.translate(
          (pv.event.pageX - i.x) * j,
          (pv.event.pageY - i.y) * j
        );
        if (k) {
...
pv.Geo.projections = {
      ...
      return {
        ...
        y:
          b.lat > 85
            ...
            : b.lat < -85
            ? -1
            : Math.log(Math.tan(Math.PI / 4 + pv.radians(b.lat) / 2)) / Math.PI,
      };
    },
    invert: function (b) {
      return {
  ...
  aitoff: {
    project: function (b) {
      ...
      return {
        x:
          (2 * (d ? (Math.cos(b) * Math.sin(c / 2) * d) / Math.sin(d) : 0)) /
          Math.PI,
...
pv.Geo.scale = function (b) {
  ...
  c.projection = function (m) {
    if (arguments.length) {
      j =
        typeof m == 'string'
          ? pv.Geo.projections[m] || pv.Geo.projections.identity
          : m;
