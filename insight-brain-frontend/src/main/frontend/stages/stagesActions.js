...
export function fetchStageTypes(purpose) {
  return function(dispatch, getState) {
    ...
    if (!getState().stages[purpose].stageTypes) {
      ...
      return axios.get(urlsByPurpose[purpose])
          .then(({ data }) => { dispatch(fetchStageTypesFulfilled(purpose, data)); })
          .catch(error => { dispatch(fetchStageTypesFailed(purpose, error)); });
    }
    else {
