...
describe('InnerSourceRepositoryTile', () => {
  ...
  it('renders loading indicator and handles error', async () => {
    ...
    expect(await screen.findByText('InnerSource Repositories')).toBeVisible();
  });

  it('renders list of configured repositories', () => {
    renderComponent();

    repositoryConnectionRequestPayload.repositoryConnections.forEach(async (repository) => {
      expect(await screen.findByText(repository.baseUrl)).toBeVisible();
      expect(screen.getByText(repository.format)).toBeVisible();
    });
  });
