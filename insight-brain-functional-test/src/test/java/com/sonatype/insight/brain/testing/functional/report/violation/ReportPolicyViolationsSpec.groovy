...
extends BaseSpec
{
  ...
  private static Policy createNoLGPL(String license, String name) {
    Policy policy = new Policy(null, "No" + name);

    Constraint constraint = new Constraint(null, "No" + name, LogicalOperator.AND);
    constraint.addCondition(new Condition(new LicenseConditionType().getId(), "is", license));
  ...
  private static Policy createNoSVPolicy() {
    Policy policy = new Policy(null, "NoSV");

    Constraint constraint = new Constraint(null, "NoSVC", LogicalOperator.AND);
    constraint.addCondition(new Condition(new SecurityVulnerabilityConditionType().getId(), "present"));
    policy.setConstraints(Collections.singletonList(constraint));
    policy.setOwnerId(app.getId());
  ...
  private static boolean hasRow(List<PolicyReportRow> rows, String coordinates, boolean waived = false) {
    for (PolicyReportRow row : rows) {
      if (row.coordinates == coordinates && row.waived.isPresent() == waived) {
        return true;
      }
    }
    return false;
