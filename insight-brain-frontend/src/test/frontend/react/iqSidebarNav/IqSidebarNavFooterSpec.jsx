...
describe('IqSidebarNavFooter', function () {
  ...
  it('renders a footer with supplied props', function () {
    ...
    expect(getShallowComponent({ productName: 'product' }).find('.iq-sidebar-nav-footer__product-name')).toHaveText(
      'product'
    );
    expect(getShallowComponent().find('.iq-sidebar-nav-footer__release-number')).not.toExist();
    expect(getShallowComponent({ releaseNumber: '10x' }).find('.iq-sidebar-nav-footer__release-number')).toHaveText(
      'Release 10x'
    );

    expect(getShallowComponent().find('.iq-sidebar-nav-footer__powered')).toHaveText('Powered by Nexus IQ Server');
    expect(getShallowComponent().find('.iq-sidebar-nav-footer__created')).toHaveText('Created by Sonatype');
  });
});
