...
const renderVersionGraph = (function() {
  var defaults = {
      ...
      derivedValues = {
        ...
        left: function(config) {
          ...
          // calculate the point in the inner chart where we need to start drawing, is based off having the current
          // version centered in the chart
  ...
  function createHighlights(vizContent, config) {
    ...
    componentInfoBars.data(config.data.versions).width(config.spacer - 1).left(function() {
      ...
      // Shennanigans to ensure Protovis creates invisible elements that have listeners attached
      return pv.color('rgba(255, 255, 255,0.1)');
    }).lineWidth(1);
  ...
  function createPopularityPanel(vis, config) {
    ...
    inner.add(pv.Bar).data(data.versionPopularity).width(config.barWidth).left(function() {
      ...
      return (barHeight * d * maxValue) + 3;
        ...
        function() {
          return this.index < data.currentVersionIndex ? '#a8a9ad'
            : this.index >= nextMajorRevIndex ? '#6d97d0'
              : this.index === data.currentVersionIndex ? '#58585a'
                : '#8bc73e';
        });
  ...
  function loadVersionChart(config) {
    ...
    vizContent = new pv.Panel().canvas('aiVersionChartViz').overflow('hidden').height(config.actualHeight).width(
        config.contentWidth).fillStyle(bgBlue).strokeStyle(bgBlue);
...
function panToXIndex(val) {
  ...
  let m = componentInfoVizContent.transform().translate(val, 0),
      temp = componentInfoXIndex + val;
  // Only pan the graph if the new x index is one of the last two displayed versions on either side of the graph
  // currently. Or if the new x index is not on the display at all.
