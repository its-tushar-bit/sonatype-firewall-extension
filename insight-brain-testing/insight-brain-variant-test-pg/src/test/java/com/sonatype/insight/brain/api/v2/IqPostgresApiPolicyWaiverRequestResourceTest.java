...
@IqPostgresTest
class IqPostgresApiPolicyWaiverRequestResourceTest
{
  ...
  @Test
  void testAddPolicyWaiverRequestByPolicyViolationId_NoRequestBody() throws Exception {
    Application app = ctx.tempEntity().newApplicationWithParent();
    Policy policy = ctx.tempEntity().newPolicy(app);
  ...
  void testReviewPolicyWaiverRequest_RepositoryContainer() throws Exception {
    ...
    Policy policy = ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID);
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(containerImageApp.getId(), ProxyStageType.ID, "scanIdContainer");
    ...
    User reviewer =
        ctx.tempEntity().newUser("reviewerUsername", "reviewerFirstName", "reviewerLastName", "reviewer@example.com");
    Role role = ctx.tempEntity().newRole(false /* global */, Permission.WAIVE_POLICY_VIOLATIONS);
    ctx.tempEntity()
        .newMembershipMapping(RepositoryContainer.REPOSITORY_CONTAINER_ID, role.getId(), reviewer.getUsername());
      ...
      AbstractPolicyViolation policyViolation) throws Exception
  {
    ...
    User reviewer =
        ctx.tempEntity().newUser("reviewerUsername", "reviewerFirstName", "reviewerLastName", "reviewer@example.com");
    Role role = ctx.tempEntity().newRole(false /* global */, Permission.WAIVE_POLICY_VIOLATIONS);
    ctx.tempEntity().newMembershipMapping(ownerId, role.getId(), reviewer.getUsername());
    ApiPolicyWaiverRequestReviewDTO apiPolicyWaiverRequestReviewDTO = new ApiPolicyWaiverRequestReviewDTO();
    apiPolicyWaiverRequestReviewDTO.comment = "updated waiver comment";
    Date updatedExpiryDate = DateUtils.addDays(new Date(), 2);
    apiPolicyWaiverRequestReviewDTO.expiryTime = updatedExpiryDate;
  ...
  @Test
  void testReviewPolicyWaiverRequest_Reject() throws Exception {
    Application app = ctx.tempEntity().newApplicationWithParent();
    Policy policy = ctx.tempEntity().newPolicy(app);
    ...
    User reviewer =
        ctx.tempEntity().newUser("reviewerUsername", "reviewerFirstName", "reviewerLastName", "reviewer@example.com");
    Role role = ctx.tempEntity().newRole(false /* global */, Permission.WAIVE_POLICY_VIOLATIONS);
    ctx.tempEntity().newMembershipMapping(app.getId(), role.getId(), reviewer.getUsername());
    ApiPolicyWaiverRequestReviewDTO apiPolicyWaiverRequestReviewDTO = new ApiPolicyWaiverRequestReviewDTO();
  ...
  @Test
  void testGetPolicyWaiverRequest_Application() throws Exception {
    Application app = ctx.tempEntity().newApplicationWithParent();
    Policy policy = ctx.tempEntity().newPolicy(app);
  ...
  @Test
  void testGetPolicyWaiverRequest_Organization() throws Exception {
    Organization org = ctx.tempEntity().newOrganization();
    Application app = ctx.tempEntity().newApplication(org.getId());
    Policy policy = ctx.tempEntity().newPolicy(app);
  ...
  @Test
  void testGetPolicyWaiverRequest_Repository() throws Exception {
    Repository repository = ctx.tempEntity().newRepository();
    Policy policy = ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID);
  ...
  @Test
  void testGetPolicyWaiverRequest_RepositoryManager() throws Exception {
    Repository repository = ctx.tempEntity().newRepository();
    RepositoryManager repositoryManager = repositoryManagerDAO.getById(repository.getRepositoryManagerId());
    Policy policy = ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID);
  ...
  @Test
  void testGetPolicyWaiverRequest_RepositoryContainer() throws Exception {
    Repository repository = ctx.tempEntity().newRepository();
    Policy policy = ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID);
  ...
  void testWithdrawPolicyWaiverRequest_Application() throws Exception {
    ...
    Policy policy = ctx.tempEntity().newPolicy(app);
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanIdApp");
    PolicyViolation policyViolation = ctx.tempEntity().newPolicyViolation(policyEvaluation, policy);
  ...
  void testWithdrawPolicyWaiverRequest_Organization() throws Exception {
    ...
    Policy policy = ctx.tempEntity().newPolicy(app);
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanIdOrg");
    PolicyViolation policyViolation = ctx.tempEntity().newPolicyViolation(policyEvaluation, policy);
  ...
  @Test
  void testWithdrawPolicyWaiverRequest_DifferentUser_ReturnsNotFound() throws Exception {
    Application app = ctx.tempEntity().newApplicationWithParent();
    Policy policy = ctx.tempEntity().newPolicy(app);
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanIdOther");
    PolicyViolation policyViolation = ctx.tempEntity().newPolicyViolation(policyEvaluation, policy);
    ...
    User otherUser = ctx.tempEntity().newUser("otherUsername", "Other", "User", "other@example.com");
    Role readRole = ctx.tempEntity().newRole(false /* global */, Permission.READ);
    ctx.tempEntity().newMembershipMapping(app.getId(), readRole.getId(), otherUser.getUsername());
  ...
  @Test
  void testWithdrawPolicyWaiverRequest_AlreadyApproved_ReturnsBadRequest() throws Exception {
    Application app = ctx.tempEntity().newApplicationWithParent();
    Policy policy = ctx.tempEntity().newPolicy(app);
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanIdApp");
  ...
  @Test
  void testWithdrawPolicyWaiverRequest_AlreadyRejected_ReturnsBadRequest() throws Exception {
    Application app = ctx.tempEntity().newApplicationWithParent();
    Policy policy = ctx.tempEntity().newPolicy(app);
    PolicyEvaluation policyEvaluation =
        ctx.tempEntity().newPolicyEvaluation(app.getId(), BuildStageType.ID, "scanIdApp");
  ...
  @Test
  void testWithdrawPolicyWaiverRequest_SecondCall_ReturnsNotFound() throws Exception {
    ...
    HttpResponse first = restRequest().path(POLICY_WAIVER_REQUEST_ID_PATH)
        .parameter(app.getType(), app.getId(), policyWaiverRequestId)
        .delete();
    ctx.assertResponseStatus(204, first);

    HttpResponse second = restRequest().path(POLICY_WAIVER_REQUEST_ID_PATH)
        .parameter(app.getType(), app.getId(), policyWaiverRequestId)
        .delete();
    ctx.assertResponseStatus(404, second);
    assertThat(policyWaiverRequestDAO.getById(policyWaiverRequestId)).isNull();
  }

  @Test
  void testWithdrawPolicyWaiverRequest_AllowsResubmissionForSameViolation() throws Exception {
  ...
  private void approveAsReviewer(OwnerType ownerType, String ownerId, String policyWaiverRequestId) throws Exception {
    User reviewer = ctx.tempEntity()
        .newUser("reviewerForWithdraw_" + policyWaiverRequestId.substring(0, 8),
  ...
  private void rejectAsReviewer(OwnerType ownerType, String ownerId, String policyWaiverRequestId) throws Exception {
    User reviewer = ctx.tempEntity()
        .newUser("reviewerForReject_" + policyWaiverRequestId.substring(0, 8),
            "reviewerFirstName", "reviewerLastName",
  ...
  @Test
  void testGetPolicyWaiverRequests_httpEndpoint_issuesBoundedSelectCount() throws Exception {
    Assumptions.assumeTrue(JooqSqlCounterListener.getInstance().isEnabled(),
        "Enable with -DargLine=\"-DcustomMetrics=sqlcount\"");
