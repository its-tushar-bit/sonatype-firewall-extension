...
describe('Unscanned components modal table', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(applicationReportSelectors, 'selectUnscannedComponents').and.returnValue(unscannedComponents);
    renderComponent = () => {
  ...
  it('sorts the table rows by component name', () => {
    ...
    expect(tableRows[1].innerText.replace(lineBreakRegexp, '')).toBe('firstFilenamefirst/test');

    fireEvent.click(sortButton);
    tableRows = screen.getAllByRole('row');
    expect(tableRows[1].innerText.replace(lineBreakRegexp, '')).toBe('secondFilenamesecond/test');
  });
});
