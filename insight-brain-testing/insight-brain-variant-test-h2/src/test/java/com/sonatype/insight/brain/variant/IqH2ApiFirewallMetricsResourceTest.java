...
import com.sonatype.insight.brain.api.v2.ApiFirewallMetricsResource;
...
@IqH2Test
class IqH2ApiFirewallMetricsResourceTest
{
  private IqTestContext ctx;

  private HttpRequest restRequest() {
    return ctx.restRequest().path(ApiFirewallMetricsResource.RESOURCE_PATH);
  }

  @Test
  void testGetFirewallMetrics() throws Exception {
    LocalDate fiveDaysAgoLocalDate = LocalDate.now().minusDays(5);
    ctx.tempEntity()
        .newFirewallMetrics(FirewallMetricsName.WAIVED_COMPONENTS, 10,
  ...
  @Test
  void testGetRoiFirewallMetrics_RoiConfigurationExist() throws Exception {
    ctx.tempEntity()
    ...
    ctx.tempEntity().newFirewallMetrics(FirewallMetricsName.SUPPLY_CHAIN_ATTACKS_BLOCKED, 10, new Date());
    ctx.tempEntity().newFirewallMetrics(NAMESPACE_ATTACKS_BLOCKED, 10, new Date());
    ctx.tempEntity().newFirewallMetrics(SAFE_VERSIONS_SELECTED_AUTOMATICALLY, 10, new Date(), LocalDate.now());
