...
/**
 * Tests the REST endpoint for GitHub App installation setup callback. Uses WireMock to mock GitHub OAuth
 * and API endpoints.
 */
@IqH2Test
class IqH2ApiGitHubAppResourceTest
{
  ...
  private com.sonatype.insight.brain.HttpRequest restRequest() {
    return ctx.restRequest().path(PublicApiPaths.GITHUB_APP_RESOURCE_PATH);
  }
  ...
  @Test
  void testHandleInstallationSetup_Success() throws Exception {
  ...
  @Test
  void testHandleInstallationSetup_InvalidStateToken() throws Exception {
    HttpResponse response = restRequest()
        .path("setupInstallation")
        .query("installation_id", INSTALLATION_ID.toString())
  ...
  @Test
  void testHandleInstallationSetup_ExpiredStateToken() throws Exception {
  ...
  @Test
  void testHandleInstallationSetup_UserDoesNotOwnInstallation() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleInstallationSetup_GitHubOAuthFailure_BadRequest() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleInstallationSetup_GitHubOAuthFailure_Unauthorized() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleInstallationSetup_GitHubInstallationsFailure_ServerError() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleInstallationSetup_MultipleInstallations_VerifyCorrectOne() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleInstallationSetup_EmptyInstallationsList_ShouldFail() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleRedirect_Success() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleRedirect_InvalidStateToken() throws Exception {
    HttpResponse response = restRequest()
        .path("redirect")
        .query("code", MANIFEST_CODE)
        .query("state", "invalid-state-token")
  ...
  @Test
  void testHandleRedirect_ExpiredStateToken() throws Exception {
    Date pastDate = new Date(System.currentTimeMillis() - 60000);
  ...
  @Test
  void testHandleRedirect_MissingCode() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleRedirect_MissingState() throws Exception {
    HttpResponse response = restRequest()
        .path("redirect")
        .query("code", MANIFEST_CODE)
        .get();
  ...
  @Test
  void testHandleRedirect_GitHubManifestConversionFailure_BadRequest() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleRedirect_GitHubManifestConversionFailure_ServerError() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
  ...
  @Test
  void testHandleRedirect_StateTokenConsumedOnce() throws Exception {
    Date futureDate = new Date(System.currentTimeMillis() + 900000);
    ...
    ctx.assertResponseStatus(500, response1);
    ...
    ctx.assertResponseStatus(400, response2);
  ...
  private GitHubAppInstallationState createInstallationState(String stateValue, String githubAppId, Date expiresAt) {
    return ctx.tempEntity().newGitHubAppInstallationState(stateValue, githubAppId, generateCodeVerifier(), expiresAt);
  }

  private GitHubAppRegistrationState createRegistrationState(String stateToken, String ownerId, Date expiresAt) {
    return ctx.tempEntity().newGitHubAppRegistrationState(stateToken, ownerId, expiresAt);
  }
  ...
  @Test
  void testListGitHubApps_returnsAllDirectApps() throws Exception {
  ...
  @Test
  void testListGitHubApps_excludesInheritedApps() throws Exception {
    Organization childOrg = ctx.tempEntity().newOrganization("child-org", organization);
  ...
  @Test
  void testListGitHubApps_emptyWhenNoApps() throws Exception {
    Organization emptyOrg = ctx.tempEntity().newOrganization("empty-org");
  ...
  @Test
  void testDeleteGitHubApp_removesApp() throws Exception {
    HttpResponse response = restRequest()
    ...
    HttpResponse listResponse = restRequest()
        .query("ownerId", organization.getId())
        .get();
    ctx.assertResponseStatus(200, listResponse);
    assertThat(listResponse.getBodyText()).isEqualTo("[]");
  }

  @Test
  void testDeleteGitHubApp_rejectsWrongOwner() throws Exception {
    Organization otherOrg = ctx.tempEntity().newOrganization("other-org");
  ...
  @Test
  void testDeleteGitHubApp_returnsNotFoundForMissingApp() throws Exception {
    HttpResponse response = restRequest()
        .path("nonexistent-id")
        .query("ownerId", organization.getId())
        .delete();
  ...
  private GitHubApp createAdditionalGitHubApp(String ownerId, Long installationId, boolean isActive) {
    ...
    return ctx.tempEntity().newGitHubApp(app);
  }
