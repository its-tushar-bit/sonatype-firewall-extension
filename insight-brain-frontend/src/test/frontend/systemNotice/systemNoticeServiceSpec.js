...
describe('systemNoticeServiceSpec.js', function() {
  beforeEach(angular.mock.module(systemNoticeModule.name, clmLocationModule.name));
  ...
  describe('getting the system notice', function() {
    it('returns it when the request succeeds', function() {
      ...
      $httpBackend.expectGET(CLMLocations.getSystemNoticeFetchUrl()).respond(
          SystemNoticeMockData.getSystemNotice('message', true));
      $httpBackend.flush();

      expect(successSpy).toHaveBeenCalledWith(SystemNoticeMockData.getSystemNotice('message', true));
      expect(failSpy).not.toHaveBeenCalled();
    });

    it('throws a failed request', function() {
      systemNoticeService.getSystemNotice().then(successSpy).catch(failSpy);
      $httpBackend.expectGET(CLMLocations.getSystemNoticeFetchUrl()).respond(404, 'not found');
      $httpBackend.flush();
  ...
  it('returns the default system notice on get default system notice', function() {
    ...
    expect(defaultSystemNotice.message).toEqual('Error: could not get the system notice from the server');
  ...
  describe('saving the system notice and putting it in the request', function() {
    it('returns no content when the request succeeds', function() {
      ...
      systemNoticeService.saveSystemNotice(systemNotice).then(successSpy).catch(failSpy);
      $httpBackend.expectPUT(CLMLocations.getSystemNoticeUrl(), systemNotice).respond(204, 'no content');
      $httpBackend.flush();
    ...
    it('throws a failed request', function() {
      var systemNotice = SystemNoticeMockData.getSystemNotice('message', true);
      systemNoticeService.saveSystemNotice(systemNotice).then(successSpy).catch(failSpy);
      $httpBackend.expectPUT(CLMLocations.getSystemNoticeUrl(), systemNotice).respond(401, 'unauthorized');
      $httpBackend.flush();
