    ...
    extends AbstractServletClient<ReportClient>
{
  ...
  public ServletResult embedReport(final String path) throws IOException {
    return path("rest/report", appId, scanId, "embedReport", path).get();
  }

  public ServletResult printReport(final String projectName, final int buildNumber) throws IOException {
  ...
  public ServletResult auditLog(final String path, final String jsonKey) throws IOException {
    final String[] params = { "key", jsonKey };
    return path("rest/report", appId, scanId, "auditLog", path).query(params).get();
  }

  public ServletResult handleRelativeToReport(final String path, final Map<String, String[]> query) throws IOException {
