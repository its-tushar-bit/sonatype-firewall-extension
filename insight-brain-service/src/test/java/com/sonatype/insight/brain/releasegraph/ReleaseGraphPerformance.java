...
public class ReleaseGraphPerformance
{
    ...
    private LoadingCache<ReleaseGraphKey, byte[]> cache =
        CacheBuilder.newBuilder().maximumSize( 1000 ).build( new ReleaseGraphCacheLoader() );

    private ReleaseGraphPerformance( int threads, InsightWork work )
        throws Exception
    {
        callables = new LinkedList<UserCallable>();
        pool =
            new ThreadPoolExecutor( threads, threads, 1, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>( threads ) );
     ...
     * @throws Exception
     */
    ReleaseGraphPerformance( int reports, int usersPerReport, int connectionsPerUser, InsightWork work )
        throws Exception
    {
    ...
    ReleaseGraphPerformance( int users, boolean preload, InsightWork work )
        throws Exception
    {
        this( users, work );
    ...
    List<Map<GAVPopularity, Long>> begin()
        throws Exception
    {
        try
        {
    ...
    private static void doOutput( String file, List<Map<GAVPopularity, Long>> results )
        throws IOException
    {
    ...
    private static InsightWork createInsightWork()
        throws IOException
    {
    ...
    private String createReport( InsightWork work )
        throws IOException
    {
    ...
    private List<GAVPopularity> getGAVs()
        ...
        implements Callable<Map<GAVPopularity, Long>>
    {
        ...
        @Override
        public Map<GAVPopularity, Long> call()
            throws Exception
        {
            for ( GAVPopularity gav : gavs )
            {
