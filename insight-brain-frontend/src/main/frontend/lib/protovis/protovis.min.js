...
try {
  ...
  pv.parse = function (b) {
    for (
      var c = new RegExp('function\\s*(\\b\\w+)?\\s*\\([^)]*\\)\\s*', 'mg'), d, f, g = 0, h = '';
      (d = c.exec(b));
...
pv.Scale.quantitative = function () {
  ...
  c.range = function () {
    if (arguments.length) {
      ...
      h = [];
      for (var n = 0; n < g.length - 1; n++) h.push(pv.Scale.interpolator(g[n], g[n + 1]));
      return this;
    }
    return g;
  };
...
pv.Scale.log = function () {
  ...
  b.ticks = function () {
    var h = b.domain(),
      ...
      l = [];
    if (i) for (l.push(-g(-j)); j++ < k; ) for (i = c - 1; i > 0; i--) l.push(-g(-j) * i);
    else {
...
pv.SvgScene.pathBasis = (function () {
  ...
  d.segment = function (f, g, h, i) {
    var j = b(c[0], f, g, h, i),
      ...
      l = b(c[2], f, g, h, i);
    f = b(c[3], f, g, h, i);
    return 'M' + j.x + ',' + j.y + 'C' + k.x + ',' + k.y + ',' + l.x + ',' + l.y + ',' + f.x + ',' + f.y;
  };
  return d;
})();
...
pv.SvgScene.curveHermite = function (b, c) {
  ...
  if (d) {
    b = b[k];
    f += 'Q' + (h.left + (j.x * 2) / 3) + ',' + (h.top + (j.y * 2) / 3) + ',' + b.left + ',' + b.top;
  }
  return f;
};
...
pv.SvgScene.area = function (b) {
  ...
  for (var j = [], k, l = 0; l < b.length; l++) {
    ...
    if (k.width || k.height) {
      ...
      j.push(
        (q - l > 2 && (g.interpolate == 'basis' || g.interpolate == 'cardinal' || g.interpolate == 'monotone') ? d : c)(
          l,
          q - 1
        )
      );
        ...
        c = this.expect(
          ...
          'text',
          {
            ...
            transform:
              ...
              f.top +
              ')' +
              (f.textAngle ? ' rotate(' + (180 * f.textAngle) / Math.PI + ')' : '') +
  ...
  for (var g = b.length - 1; d < g; d++) {
    ...
    if (h.visible && i.visible) {
      ...
      if (j.opacity) {
        ...
        } else i = f ? f[d] : 'M' + h.left + ',' + h.top + this.pathSegment(h, i);
        c = this.expect(c, 'path', {
...
pv.SvgScene.pathJoin = function (b, c, d, f) {
  ...
  return 'M' + c.x + ',' + c.y + 'L' + k.x + ',' + k.y + ' ' + l.x + ',' + l.y + ' ' + j.x + ',' + j.y;
};
    ...
    if (g.visible) {
      if (!b.parent) {
        ...
        if (!c) {
          ...
          for (var h = 0; h < this.events.length; h++) c.addEventListener(this.events[h], this.dispatch, false);
          d = c.firstChild;
        }
        b.$g = c;
        ...
        if (k >= 2 * Math.PI)
          i = i
            ? 'M0,' +
              ...
              i +
              'Z'
            : 'M0,' + j + 'A' + j + ',' + j + ' 0 1,1 0,' + -j + 'A' + j + ',' + j + ' 0 1,1 0,' + j + 'Z';
        else {
...
a.bind = function () {
  ...
  d[3].reverse();
  var h = this;
  do for (var i in h.properties) i in c || d[2].push((c[i] = { name: i, type: 2, value: null }));
  while ((h = h.proto));
  h = d[0].concat(d[1]);
    ...
    if (c) {
      ...
      if (!(c = d[this.index])) {
        ...
        if (this.$dom) this.$dom.parentNode.insertBefore(c, this.$dom);
          ...
          for (d = document.body; d.lastChild && d.lastChild.tagName; ) d = d.lastChild;
...
pv.Layout.Force.prototype.buildImplied = function (b) {
  ...
  if (pv.Layout.Network.prototype.buildImplied.call(this, b)) {
    ...
    for (
      var d = this, f = b.nodes, g = b.links, h = b.iterations, i = b.width, j = b.height, k = 0, l;
      k < f.length;
      k++
    ) {
      l = f[k];
    ...
    l.force(pv.Force.charge(b.chargeConstant).domain(b.chargeMinDistance, b.chargeMaxDistance).theta(b.chargeTheta));
...
// Sonatype customization: ES6 compat
export default pv;
