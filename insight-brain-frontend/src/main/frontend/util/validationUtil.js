...
export const validateHostname = (value) =>
  isValidHostname(value) ? null : 'Invalid host name';

export const validateNonEmpty = (val) =>
  val && val.length ? null : 'Must be non-empty';

export const validateMaxLength = curryN(
  2,
  function validateMaxLength(maxLength, val) {
    if (!val || !maxLength) {
      return null;
    }
...
export const validatePatternMatch = curryN(
  3,
  function validatePatternMatch(regex, message, val) {
 ...
 */
export const combineValidators = (validators) => (val) =>
  reject(isNil, map(applyTo(val), validators));
