      ...
      DrawListener drawListener) throws IOException
  {
    ...
    for (StyledText styledText : styledTextList) {
      ...
      if (styledText.getText().length() > 0) {
         ...
         * styledText.getText());
         */
        PdfGeneratorUtils.addText(contentStream, styledText.getFontDescriptor().getFont(),
            styledText.getFontDescriptor().getSize(), styledText.getText());
        // END PATCH
      }
