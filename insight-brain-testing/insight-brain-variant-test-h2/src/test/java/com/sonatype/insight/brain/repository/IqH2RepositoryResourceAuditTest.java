...
/**
 * Reused-server H2 port of the legacy {@code RepositoryResourceAuditTest} (composition via the injected
 * {@link IqTestContext}; shares the module's single {@code TestCLMServer}).
 */
@IqH2Test
    ...
    implements AuditTestSupport
{
  ...
  public void testGetPolicyViolations() throws Exception {
    ...
    ProxyRepositoryComponent proxyRepositoryComponent =
        ctx.tempEntity().newRepositoryComponent(repository.getId(), "testPath");
  ...
  public void testGetPolicyViolation() throws Exception {
    ...
    ProxyRepositoryPolicyViolation proxyRepositoryPolicyViolation =
        ctx.tempEntity().newRepositoryPolicyViolation(repository.getId());
  ...
  public void testConfigureRepositories_RuntimeException() throws Exception {
    RepositoryManager repositoryManager = ctx.tempEntity().newRepositoryManager();
    Repository existingRepository1 = ctx.tempEntity().newRepository(repositoryManager, "testRepoName1");
    Repository existingRepository2 = ctx.tempEntity().newRepository(repositoryManager, "testRepoName2");
  ...
  public void testConfigureRepositories_ExistingRepository() throws Exception {
    ...
    Repository repository =
        ctx.tempEntity().newRepository(repositoryManager, "testRepoMaven", RepositoryType.proxy, "maven");
  ...
  private Repository repositoryWithComponents(int componentCount) {
    ...
    for (int i = 0; i < componentCount; i++) {
      ctx.tempEntity().newRepositoryComponent(repository, COMPONENT_HASH);
    }
