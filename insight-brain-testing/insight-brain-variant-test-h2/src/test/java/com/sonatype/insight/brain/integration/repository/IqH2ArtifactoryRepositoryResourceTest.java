...
/**
 * H2 port of {@code ArtifactoryRepositoryResourceTest} plus every {@code @Test} inherited (via
 * {@code AbstractRepositoryResourceTest}) from the legacy JUnit4 base class.
 */
@IqH2Test
class IqH2ArtifactoryRepositoryResourceTest
{
  ...
  void setUp() throws Exception {
    ...
    ctx.setFeatures(LicensedFeature.FIREWALL_FOR_ARTIFACTORY);
  }

  private HttpRequest restRequest() {
    // Integration REST endpoints don't use/require CSRF
    return ctx.restRequest().path(ArtifactoryRepositoryResource.RESOURCE_PATH).noCsrfToken();
  }
