...
Run with default configuration from the command line with:
    java -Djava.awt.headless=true -jar sonatype-clm-server-*.jar
...
Optionally provide a configuration file and run as:
    java -Djava.awt.headless=true -jar sonatype-clm-server-*.jar server config.yml
...
This can be set when invoking java, with a Sun/Oracle JVM:
    java -XX:MaxPermSize=128m -Djava.awt.headless=true -jar sonatype-clm-server-*.jar server config.yml
