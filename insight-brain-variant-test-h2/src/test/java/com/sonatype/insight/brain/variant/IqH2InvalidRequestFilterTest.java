...
@IqH2Test
class IqH2InvalidRequestFilterTest
{
  private IqTestContext ctx;

  @Test
  void testBackslashSemicolonBlockedByDefault() throws Exception {
    HttpResponse response = ctx.restRequest().path("any/thing/\\after-backslash").get();
    ctx.assertResponseStatus(400, response);

    response = ctx.restRequest().path("any/thing/;after-semicolon").get();
    ctx.assertResponseStatus(400, response);
  }

  @Test
  void testNonAsciiAllowedByDefault() throws Exception {
    // Jetty 10 handles this now rather than InvalidRequestFilter
    assertThat(doRequestWithNonAsciiCharacters()).isEqualTo(400);
  }

  @Test
  void testBackslashEnabled() throws Exception {
  ...
  @Test
  void testNonAsciiEnabled() throws Exception {
    ApiConfigurationService configurationService = ctx.lookup(ApiConfigurationService.class);
  ...
  @Test
  void testNonAsciiDisabled() throws Exception {
    ApiConfigurationService configurationService = ctx.lookup(ApiConfigurationService.class);
  ...
  @Test
  void testSemicolonEnabled() throws Exception {
    ApiConfigurationService configurationService = ctx.lookup(ApiConfigurationService.class);
  ...
  private int doRequestWithNonAsciiCharacters() throws IOException {
    ...
    URL url = new URL(ctx.restBaseUrl() + "any/thing/non-ascii/\u007F/газета");
    HttpURLConnection connection = (HttpURLConnection) url.openConnection();
    connection.setRequestMethod("GET");

    // The reused server's fixed test credentials (admin/admin123); see AbstractBaseIntegrationTest.
    String authenticationString = ctx.getUsername() + ":admin123";
