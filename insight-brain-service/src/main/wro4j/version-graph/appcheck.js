/**
 * @license Copyright (c) 2011-2014 Sonatype, Inc. All rights reserved.
...
(function() {
  ...
  function barChart(data, config) {
    ...
    vis.add(pv.Label)
        ...
        .event('mouseover',
            pv.Behavior.tipsy({fade: true, gravity: 'w', html: true, opacity: 1.0, offset: 35, title: function() {
              return 'Sonatype utilizes the base score from the Common Vulnerability Scoring System, version 2.';
            }}))
        .textAlign('left')
        .textBaseline('middle');
    ...
    if (noSV) {
      _createLabel(vis, 'No Vulnerabilities Found',
          { width: config.width, height: config.height, rightPadding: rightPadding, leftPadding: leftPadding });
    }
    vis.render();
  }
