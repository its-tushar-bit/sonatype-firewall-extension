...
describe('DeleteModalReduxController', function() {
  ...
  function createController(resourceType, resourceName, headerText, bodyText, maskText, continueAction,
                            cancelAction, stateMapper) {
  ...
  describe('watcher of vm.deleting and vm.success', function() {
    it('calls vm.deleteResourceMask.showSuccessMaskBriefly when vm.success is true and vm.deleting is false',
        function() {
          var scope = createController(),
              vm = scope.vm;
  ...
  describe('mapStateToThis', function() {
    ...
    it('sets the error from the errorState property that comes out of the stateMapper, run through ' +
        'Message.getHttpErrorMessage', inject(function(Messages) {
      var stateMapper = function(x) { return x; };
