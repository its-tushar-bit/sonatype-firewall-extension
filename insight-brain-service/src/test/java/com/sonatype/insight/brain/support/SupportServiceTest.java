    ...
    extends AbstractComponentTest
{
  ...
  public void testCreateSupportZip() throws Exception {
    assertThat(supportService.createSupportZip(), notNullValue());
  }
  ...
  public void testCreateSupportZipFilenameRules() throws Exception {
    ...
    final File firstZip = supportService.createSupportZip();
    ...
    final File secondZip = supportService.createSupportZip();
  ...
  public void testCreateSupportZip_UsesSubDir() throws Exception {
    supportService.createSupportZip();
    ...
    try {
      ...
      try (final ZipFile zipFile = new ZipFile(supportZip)) {
        ...
        assertThat(zipEntry.getName(),
            is(getZipFileBasename(supportZip) + "/" + SupportFileType.CONFIG.dirName + "/filtered-" +
                CONFIG_YML_FILENAME));
  ...
  private static void verifyRequiredEntries(final File supportZipFile, final Enumeration<? extends ZipEntry> entries) {
    ...
    assertThat(entries.nextElement().getName(),
        is(zipFileBasename + "/" + SupportFileType.INFO.dirName + "/sysinfo.json"));
    assertThat(entries.nextElement().getName(),
        is(zipFileBasename + "/" + SupportFileType.INFO.dirName + "/product-version.json"));
    assertThat(entries.nextElement().getName(),
        is(zipFileBasename + "/" + SupportFileType.INFO.dirName + "/product-license.json"));
    assertThat(entries.nextElement().getName(),
    ...
    try {
      ...
      try (final ZipFile zipFile = new ZipFile(supportZip)) {
        ...
        if (InsightBrainService.getConfigFile() != null) {
          assertThat(entries.nextElement().getName(),
              is(getZipFileBasename(supportZip) + "/" + SupportFileType.CONFIG.dirName +
    ...
    try {
      ...
      try (final ZipFile zipFile = new ZipFile(supportZip)) {
        ...
        assertThat(entries.nextElement().toString(),
            is(getZipFileBasename(supportZip) + "/" + SupportFileType.INFO.dirName + "/sysinfo.json"));

        final ZipEntry zipEntry = entries.nextElement();
        assertThat(zipEntry.toString(),
            is(getZipFileBasename(supportZip) + "/" + SupportFileType.INFO.dirName + "/product-version.json"));
        try (final ByteArrayOutputStream zipEntryContent = new ByteArrayOutputStream()) {
          try (final InputStream zipEntryStream = zipFile.getInputStream(zipEntry)) {
            IOUtil.copy(zipEntryStream, zipEntryContent);
