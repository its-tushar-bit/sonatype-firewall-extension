...
function resetDialog(Modal, discardFn, discardLabel) {
  ...
  return function () {
    Modal.open({
      backdrop: 'static',
      template:
        '<div id="ldap-unsaved-changes"><div class="iq-modal-header"><h2>Unsaved Changes</h2></div>' +
        '<div class="iq-modal-content">There are unsaved changes, continuing will discard them.</div>' +
        '<div class="iq-modal-footer">' +
        '<button class="iq-btn iq-btn--primary" ng-click="discardChanges()">' +
        discardLabel +
        '</button>' +
...
function testRequest($scope, $http, resourceUrl, requestData) {
  ...
  $http.put(resourceUrl, requestData).then(
    function (response) {
...
export function LdapConfigurationController(
  $scope,
  $state,
  Dialog,
  ldapStore,
  ...
  $scope.canSaveEdit = function () {
    return (
      $scope.ldapEditor &&
      !$scope.ldapEditor.$invalid &&
      (($scope.ldapNameForm && $scope.ldapNameForm.$visible) ||
        ($scope.ldap && $scope.ldap.name))
    );
  };
  ...
  $scope.save = function () {
    ...
    $scope.saving = true;
    $scope.ldap.$save().then(
      function (ldapServer) {
        $scope.saving = false;
        if (
          $state.current.name === 'edit-ldap' ||
  ...
  $scope.confirmDeleteConfiguration = function () {
    Dialog.open({
      ...
      body:
        'Are you sure you want to delete this LDAP configuration? \
  ...
  $scope.doLoad = function () {
    if (isAuthorized) {
      ...
      if ($state.params.ldapId) {
        ldapStore.getById($state.params.ldapId).then(
          function (ldapServer) {
...
export function LdapConnectionController(
  $scope,
  Modal,
  $http,
  CLMContextLocations,
  ...
  $scope.clearOrRestorePasswordIfNeeded = function () {
    ...
    if (isHostnameOrPortChanged()) {
      if (
        angular.equals(
          origLdapConn.systemPassword,
          $scope.ldapConn.systemPassword
        )
      ) {
  ...
  function isHostnameOrPortChanged() {
    return (
      !angular.equals(origLdapConn.hostname, $scope.ldapConn.hostname) ||
  ...
  $scope.testConnection = function () {
    testRequest(
      $scope,
      $http,
      CLMContextLocations.getLdapConnectionTest(),
      $scope.ldapConn
    );
  };
  ...
  $scope.save = function () {
    $scope.saving = true;
    $http
      .put(CLMContextLocations.getLdapConnectionConfig(), $scope.ldapConn)
      .then(
        function (response) {
          $scope.saving = false;
  ...
  $scope.$watch('ldapConn.protocol', function (newProtocol) {
    if (
      newProtocol === 'LDAP' &&
      (!$scope.ldapConn.port || $scope.ldapConn.port === 636)
    ) {
      $scope.ldapConn.port = 389;
    } else if (
      newProtocol === 'LDAPS' &&
      (!$scope.ldapConn.port || $scope.ldapConn.port === 389)
    ) {
      $scope.ldapConn.port = 636;
    }
  });

  $http.get(CLMContextLocations.getLdapConnectionConfig()).then(
    function (response) {
      origLdapConn = response.data;
      $scope.ldapConn = angular.copy(origLdapConn);
    },
...
LdapConnectionController.$inject = [
  '$scope',
  'Modal',
  '$http',
  'CLMContextLocations',
...
export function LdapUsermappingController(
  $scope,
  Modal,
  $http,
  CLMContextLocations,
  ...
  $scope.isDirty = function () {
    return (
      $scope.ldapUserMapping &&
      !angular.equals(origLdapUserMapping, $scope.ldapUserMapping)
    );
  };
  ...
  $scope.save = function () {
    ...
    $http
      .put(
        CLMContextLocations.getLdapUserMappingConfig(),
        $scope.ldapUserMapping
      )
      .then(
        function (response) {
    ...
    Modal.open({
      ...
      resolve: {
        users: function () {
          ...
          $http
            .put(
              CLMContextLocations.getLdapUserMappingTest(),
              $scope.ldapUserMapping
            )
            .then(
              function (response) {
                ...
                users.forEach(function (user) {
                  ...
                  ['username', 'realName', 'email', 'membership'].forEach(
                    function (field) {
                      if (user[field]) {
                        user.fieldCount++;
                      }
  ...
  $scope.isGroupFieldRequired = function (groupMappingType) {
    return (
      $scope.ldapUserMapping &&
      $scope.ldapUserMapping.groupMappingType === groupMappingType
    );
  };

  $http.get(CLMContextLocations.getLdapUserMappingConfig()).then(
    function (response) {
      origLdapUserMapping = response.data;
      ...
      [
        ...
        'groupBaseDN',
      ].forEach(function (nonRequiredProp) {
        origLdapUserMapping[nonRequiredProp] =
          origLdapUserMapping[nonRequiredProp] || '';
      });
...
LdapUsermappingController.$inject = [
  '$scope',
  'Modal',
  '$http',
  'CLMContextLocations',
...
export function LdapCheckUserMappingController($scope, users) {
  $scope.users = users;
  $scope.infoText =
    'Scroll through the table and verify that the values in each column are in the correct format. ' +
