...
describe('OwnerSummary', () => {
  ...
  describe('if owner is an organization', () => {
    ...
    it('renders an alert with retry if something goes wrong', async () => {
      ...
      // After retry, error should appear again (loading state may be too brief to catch in jsdom)
    ...
    it('renders proper header with selected contact if contact is provided', async () => {
      ...
      expect(await screen.findByText('Provided Contact Display Name')).toBeInTheDocument();
    });
    ...
    it('renders insufficient permissions tree', async () => {
      ...
      expect(treeMessage).toBeInTheDocument();
    ...
    it('renders limited firewall access alert when showLimitedFirewallAccessAlert is true and in firewall mode', async () => {
      ...
      expect(
        screen.getByText(/Some data or settings may not be visible. Contact your administrator/)
      ).toBeInTheDocument();
      expect(screen.queryByText('broadcast')).not.toBeInTheDocument();
    ...
    it('hides non-SBOM pills when in SBOM Manager and sbom-continuous-monitoring-ui is enabled', async () => {
      ...
      expect(await screen.findByTestId('policies-tile')).toBeInTheDocument();
      ...
      expect(screen.getByRole('button', { name: 'Continuous monitoring' })).toBeInTheDocument();
  ...
  describe('if owner is an application', () => {
    ...
    it('renders proper header with selected contact and publicId', async () => {
      ...
      expect(await screen.findByText('a-aws 4-lll app filter')).toBeInTheDocument();
      expect(await screen.findByText('(a-aws-4-lll_app_filter)')).toBeInTheDocument();
      expect(await screen.findByText('Provided Contact Display Name')).toBeInTheDocument();
    });
  ...
  describe('and selectIsSbomManagerEnabled is', () => {
    ...
    it('true, then SBOMs, Access and Continuous Monitoring tiles are visible and no other tiles are', async () => {
      ...
      expect(await screen.findByRole('heading', { name: 'SBOMs' })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: 'SBOMs' })).toBeInTheDocument();
  ...
  describe('Public Data Sources', () => {
    ...
    it('renders pill and tile', async () => {
      ...
      expect(await screen.findByTestId('owner-pill-public-data-sources-button')).toBeInTheDocument();
      expect(await screen.findByTestId('owner-pill-public-data-sources')).toBeInTheDocument();
    });
