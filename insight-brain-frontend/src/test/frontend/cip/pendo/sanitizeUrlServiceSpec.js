...
describe('cip-loader sanitizeUrlService', function () {
  ...
  beforeEach(
    angular.mock.module(cipLoaderPendoModule.name, function ($provide) {
  ...
  it('removes the baseUrl', function () {
    expect(
      sanitizeUrlService.sanitize('http://localhost:8070/assets/foo')
    ).toBe('/assets/foo');

    baseUrl = 'https://foobar.com/iq';

    expect(
      sanitizeUrlService.sanitize('https://foobar.com/iq/assets/foo')
    ).toBe('/assets/foo');
  });
  ...
  it('hashes the application id and report id', function () {
    const url =
        'http://localhost:8070/rest/report/app1/1a2b3c4d5e6fedcba/index.html',
      expectedUrl =
        '/rest/report/172dd4a0366000604e2c4de41457aa1eb3093bb59ead22e0f1d472a2aaade094/' +
