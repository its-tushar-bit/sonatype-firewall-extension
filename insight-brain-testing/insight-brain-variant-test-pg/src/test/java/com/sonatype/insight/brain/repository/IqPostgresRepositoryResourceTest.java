...
@IqPostgresTest
class IqPostgresRepositoryResourceTest
{
  ...
  @BeforeEach
  void setUp() {
    proprietaryComponentNamePatternDAO = ctx.lookup(ProprietaryComponentNamePatternDAO.class);
  ...
  @Test
  void testUnquarantineComponent() throws Exception {
  ...
  void testGetRepositories() throws Exception {
    ...
    HttpResponse response = ctx.restRequest().path(RepositoryResource.RESOURCE_PATH).get();
  ...
  @Test
  void testReevaluateRepository() throws Exception {
    Repository repo = ctx.tempEntity().newRepository();

    HttpResponse response = ctx.restRequest()
        .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.EVALUATE_PATH)
        .parameter(repo.getId())
        .post();
  ...
  @Test
  void testReevaluateRepositoryComponent() throws Exception {
    Repository repo = ctx.tempEntity().newRepository();
    ProxyRepositoryComponent component = ctx.tempEntity().newRepositoryComponent(repo.getId());
  ...
  @Test
  void testGetPolicyEvaluationTimestamps() throws Exception {
    Repository repo = ctx.tempEntity().newRepository();
    ...
    ctx.tempEntity()
        .newRepositoryComponent(repo.getId(), MatchState.EXACT, "testPathname", "testHash", componentIdentifier,
            firstPolicyEvaluationTime, quarantineTime, unquarantineTime);

    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.POLICY_EVALUATION_TIMESTAMPS_PATH)
            .parameter(repo.getId())
  ...
  void testGetPolicyViolations() throws Exception {
    Repository repo = ctx.tempEntity().newRepository();
    Policy policy = ctx.tempEntity().newPolicy(RepositoryContainer.SINGLETON);
    ProxyRepositoryComponent proxyRepositoryComponent =
        ctx.tempEntity().newRepositoryComponent(repo.getId(), "testPathname");
    ProxyRepositoryPolicyViolation proxyRepositoryPolicyViolation = ctx.tempEntity()
        .newRepositoryPolicyViolation(
            repo.getId(), 8,
            proxyRepositoryComponent.getPathname(), false /* isWaived */, Action.ID_FAIL, policy.getId(),
            policy.getName(),
            proxyRepositoryComponent.getComponentIdentifier());

    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.POLICY_VIOLATIONS_PATH)
            .parameter(repo.getId(), proxyRepositoryComponent.getPathname())
            .get();
    ...
    ProxyRepositoryPolicyViolationDTO proxyRepositoryPolicyViolationDTO = repositoryPolicyViolationDTOs[0];
    assertThat(proxyRepositoryPolicyViolationDTO.policyViolationId).isEqualTo(proxyRepositoryPolicyViolation.getId());
    assertThat(proxyRepositoryPolicyViolationDTO.componentIdentifier.getFormat())
        .isEqualTo(proxyRepositoryComponent.getComponentIdentifier().getFormat());
    assertThat(proxyRepositoryPolicyViolationDTO.componentIdentifier.getCoordinates())
        .isEqualTo(proxyRepositoryComponent.getComponentIdentifier().getCoordinates());
    assertThat(proxyRepositoryPolicyViolationDTO.componentDisplayName.getName())
        .isEqualTo(
            ComponentDisplayNameUtil.fromIdentifier(proxyRepositoryComponent.getComponentIdentifier()).getName());
    assertThat(proxyRepositoryPolicyViolationDTO.hash).isEqualTo(proxyRepositoryPolicyViolation.getHash());
    assertThat(proxyRepositoryPolicyViolationDTO.policyId).isEqualTo(proxyRepositoryPolicyViolation.getPolicyId());
    assertThat(proxyRepositoryPolicyViolationDTO.policyName).isEqualTo(proxyRepositoryPolicyViolation.getPolicyName());
    assertThat(proxyRepositoryPolicyViolationDTO.policyOwner.ownerId)
        .isEqualTo(RepositoryContainer.REPOSITORY_CONTAINER_ID);
    assertThat(proxyRepositoryPolicyViolationDTO.policyOwner.ownerName)
        .isEqualTo(RepositoryContainer.SINGLETON.getName());
    assertThat(proxyRepositoryPolicyViolationDTO.policyOwner.ownerType)
        .isEqualTo(OwnerType.REPOSITORY_CONTAINER.toString());
    assertThat(proxyRepositoryPolicyViolationDTO.policyThreatLevel)
        .isEqualTo(proxyRepositoryPolicyViolation.getThreatLevel());
    assertThat(proxyRepositoryPolicyViolationDTO.policyThreatCategory)
        .isEqualTo(proxyRepositoryPolicyViolation.getThreatCategory());
    assertThat(proxyRepositoryPolicyViolationDTO.constraintFactsJson)
        .isEqualTo(proxyRepositoryPolicyViolation.getConstraintFactsJson());
    assertThat(proxyRepositoryPolicyViolationDTO.waived).isEqualTo(proxyRepositoryPolicyViolation.isWaived());
    assertThat(proxyRepositoryPolicyViolationDTO.policyActionTypeId)
        .isEqualTo(proxyRepositoryPolicyViolation.getActionTypeId());
    assertThat(proxyRepositoryPolicyViolationDTO.lastReported).isEqualTo(proxyRepositoryPolicyViolation.getTime());
  }

  @Test
  void testGetPolicyViolation() throws Exception {
    Repository repo = ctx.tempEntity().newRepository();
    Policy policy = ctx.tempEntity().newPolicy(RepositoryContainer.SINGLETON);
    ProxyRepositoryComponent proxyRepositoryComponent =
        ctx.tempEntity().newRepositoryComponent(repo.getId(), "testPathname");
    ...
    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.POLICY_VIOLATION_PATH)
            .parameter(repo.getId(), proxyRepositoryPolicyViolation.getId())
            .get();

    ctx.assertResponseStatus(200, response);
    ProxyRepositoryPolicyViolationDTO proxyRepositoryPolicyViolationDTO =
        response.getBody(ProxyRepositoryPolicyViolationDTO.class);
    assertThat(proxyRepositoryPolicyViolationDTO.policyViolationId).isEqualTo(proxyRepositoryPolicyViolation.getId());
    assertThat(proxyRepositoryPolicyViolationDTO.componentIdentifier.getFormat())
        .isEqualTo(proxyRepositoryComponent.getComponentIdentifier().getFormat());
  ...
  @Test
  void testUpdateProprietaryComponentNamePattern() throws Exception {
    RepositoryManager repoManager = ctx.tempEntity().newRepositoryManager();
    Repository repo =
        ctx.tempEntity()
            .newRepository(repoManager, "testPublicId", RepositoryType.hosted, ComponentIdentifier.FORMAT_MAVEN);
    ProprietaryComponentNamePattern pattern = ctx.tempEntity()
        .newProprietaryComponentNamePattern(
  ...
  @Test
  void testGetUnconfiguredRepositoryManagers() throws Exception {
    ...
    RepositoryManager configuredRepoManager = ctx.tempEntity().newRepositoryManager();
    ...
    RepositoryManager unconfiguredRepoManager = ctx.tempEntity().newRepositoryManager();
  ...
  @Test
  void testGetRepositoriesByRepositoryManagerId() throws Exception {
    RepositoryManager repositoryManager = ctx.tempEntity().newRepositoryManager();
    Repository repository1 = ctx.tempEntity().newRepository(repositoryManager, "testRepoMaven", "maven2");
    Repository repository2 =
        ctx.tempEntity().newRepository(repositoryManager, "testRepoUnsupported", "unsupportedFormat");

    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.REPOSITORIES_PATH)
  ...
  void testConfigureRepositories() throws Exception {
    RepositoryManager repositoryManager = ctx.tempEntity().newRepositoryManager();
    Repository repository = ctx.tempEntity().newRepository(repositoryManager, "testRepoName", "npm");
    ...
    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.CONFIGURE_REPOSITORIES_PATH)
            .parameter(repositoryManager.getId())
  ...
  @Test
  void testConfigureFirewallOnboarding() throws Exception {
    ...
    Policy securityMaliciousPolicy =
        ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID, "Security-Malicious");
    Policy integrityRatingPolicy = ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID, "Integrity-Rating");
    Policy securityNamespaceConflictPolicy =
        ctx.tempEntity().newPolicy(Organization.ROOT_ORGANIZATION_ID, "Security-Namespace Conflict");
    // Sanity checks
    ...
    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.CONFIGURE_FIREWALL_ONBOARDING_PATH)
  ...
  @Test
  void testGetProprietaryComponentNamePatternsByOwner() throws Exception {
    RepositoryManager repoManager1 = ctx.tempEntity().newRepositoryManager();
    Repository repo1 =
        ctx.tempEntity()
            .newRepository(repoManager1, "testPublicId", RepositoryType.hosted, ComponentIdentifier.FORMAT_MAVEN);
    ProprietaryComponentNamePattern pattern1 =
        ctx.tempEntity().newProprietaryComponentNamePattern(repo1, "testNamespacePattern1", "testNamePattern1");

    RepositoryManager repoManager2 = ctx.tempEntity().newRepositoryManager();
    Repository repo2 =
        ctx.tempEntity()
            .newRepository(repoManager2, "testPublicId", RepositoryType.hosted, ComponentIdentifier.FORMAT_MAVEN);
    ProprietaryComponentNamePattern pattern2 =
        ctx.tempEntity().newProprietaryComponentNamePattern(repo2, "testNamespacePattern2", "testNamePattern2");
  ...
  @Test
  void testGetAndSetRepositoryManagerIcon() throws Exception {
    RepositoryManager repoManager = ctx.tempEntity().newRepositoryManager();
    ...
    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.REPOSITORY_MANAGER_ICON_PATH)
    ...
    String expectedDefaultIconLocation =
        ctx.restBaseUrl().replaceFirst("/+$", "") + "/assets/img/defaulticon_repository_manager.png";
  ...
  void testGenerateIcon() throws Exception {
    HttpResponse response =
        ctx.restRequest()
            .path(RepositoryResource.RESOURCE_PATH, RepositoryResource.GENERATE_ICON_PATH)
            .parameter("hash")
            .get();
    ctx.assertResponseStatus(200, response);
