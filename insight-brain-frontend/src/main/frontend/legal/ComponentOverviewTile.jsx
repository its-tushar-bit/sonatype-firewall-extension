...
export default function ComponentOverviewTile(props) {
  ...
  const lastUpdatedObligation = obligations.length === 0 ? null :
    obligations.reduce((prev, current) => ((prev.lastUpdatedAt || 0) > (current.lastUpdatedAt || 0)) ? prev : current);

  const lastUpdatedAttribution = attributions.length === 0 ? null :
    attributions.reduce((prev, current) => ((prev.lastUpdatedAt || 0) > (current.lastUpdatedAt || 0)) ? prev : current);
  ...
  const lastUpdated = (() => {
    ...
    if (licenseLegalData.componentCopyrightLastUpdatedAt &&
    ...
    if (licenseLegalData.componentNoticesLastUpdatedAt &&
    ...
    if (licenseLegalData.componentLicensesLastUpdatedAt &&
  ...
  const getStageScanHref = (stageScan) => $state.href('applicationReport.policy',
      { publicId: applicationPublicId, scanId: stageScan.scanId });

  const createStageScan = (stageScan) => (
    <span id={'component-overview-tile-' + STAGE_NAME_TO_ID[stageScan.stageName]}
