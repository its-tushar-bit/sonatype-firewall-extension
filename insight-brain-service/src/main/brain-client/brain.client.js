	...
	window.Brain = {
		...
		'ci' : {
			...
			'getArtifactInfoUrl' : function (arg) {
				return basePath + 'rest/ci/component/details/' + encodeURIComponent(arg.appId) + '?' + param({ groupId : arg.groupId, artifactId : arg.artifactId, version : arg.version, instanceId : arg.instanceId, ts : new Date().getTime() });
			},
			/**
			...
			'getArtifactVersionInfoUrl' : function (arg) {
				return basePath + 'rest/ci/component/details/versions/' + encodeURIComponent(arg.appId) + '?' + param({ groupId : arg.groupId, artifactId : arg.artifactId, version : arg.version, instanceId : arg.instanceId });
			}
		},
		'ide' : {
			/**
