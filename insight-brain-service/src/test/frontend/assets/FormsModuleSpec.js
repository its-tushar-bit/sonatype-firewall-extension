describe('Forms module', function() {
  ...
  describe('FormMaskDelay', function() {
    var scope,
        promise,
        successSpy,
        errorSpy;
    ...
    function callPromiseError() {
      promise.then.mostRecentCall.args[1].apply(null, arguments);
    }

    function callPromiseSuccess() {
      promise.then.mostRecentCall.args[0].apply(null, arguments);
    }

    beforeEach(inject(function($rootScope, FormMaskDelay) {
      promise = { then : jasmine.createSpy('promiseThen') };
      scope = $rootScope.$new();
      FormMaskDelay.wrap(scope, promise).then(successSpy = jasmine.createSpy('successSpy'), errorSpy = jasmine.createSpy('errorSpy'));
    }));
    ...
    it('Success', inject(function($timeout) {
      expect(scope.formMaskActive).toEqual('active');

      callPromiseSuccess('resolved');

      expect(scope.formMaskActive).toEqual('successful');

      /// time has not quite passed
      $timeout.flush(799);
      expect(scope.formMaskActive).toEqual('successful');
      expect(successSpy).not.toHaveBeenCalled();

      $timeout.flush(1);
      expect(scope.formMaskActive).toBeUndefined();
      expect(successSpy).toHaveBeenCalledWith('resolved');
    }));

    it('Failure', inject(function($timeout) {
      expect(scope.formMaskActive).toEqual('active');

      callPromiseError('fail dude');
      expect(scope.formMaskActive).toBeUndefined();
      $timeout.flush(0)
      expect(errorSpy).toHaveBeenCalledWith('fail dude');
    }));
  });
});
