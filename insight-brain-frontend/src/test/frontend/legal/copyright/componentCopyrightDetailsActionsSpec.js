...
import {
  ...
  loadFilePathsOnPageUpdate, unloadCopyrightContexts
} from '../../../../main/frontend/legal/copyright/componentCopyrightDetailsActions';
import { getCopyrightContextUrl, getCopyrightFileCountUrl, getCopyrightFilePathsUrl }
...
describe('ComponentCopyrightDetailsAction', function() {
  ...
  describe('load copyright details', function() {
    ...
    it('immediately dispatches a COPYRIGHT_DETAILS_REQUEST action', function() {
      ...
      store.dispatch(loadComponentAndCopyrightDetails(
          'organization', 'org', 'componentHash', 1));
    ...
    it('immediately dispatches a COPYRIGHT_DETAILS_REQUEST & COPYRIGHT_DETAILS_FULFILLED actions ' +
      'when copyright details load', function(done) {
      store = SpecUtil.mockReduxStore(initialState);

      const copyrightFilePathsUrl = getCopyrightFilePathsUrl('organization', 'org', 'componentHash',
          'componentIdentifier', 'copyright_hash_2', 0, 15);
      let copyrightFileCountUrl = getCopyrightFileCountUrl(
          'organization', 'org', 'componentHash', 'componentIdentifier');
      ...
      mockAxiosCalls({
        get: {
          ...
          [copyrightFilePathsUrl]: Promise.resolve({data: ['path1/file1', 'path2/file2', 'path3/file3']})
        }
      });
    ...
    it('immediately dispatches a COPYRIGHT_DETAILS_REQUEST action and then COPYRIGHT_DETAILS_FAILURE action ' +
      'when copyright file count fail to load', function(done) {
      store = SpecUtil.mockReduxStore(initialState);
    ...
    it('immediately dispatches a COPYRIGHT_DETAILS_REQUEST action and then COPYRIGHT_DETAILS_FAILURE action ' +
      'when copyright paths fail to load', function(done) {
      store = SpecUtil.mockReduxStore(initialState);
  ...
  describe('load copyright contexts', function() {
    ...
    it('immediately dispatches a COPYRIGHT_CONTEXT_REQUEST action and then COPYRIGHT_CONTEXT_FULFILLED action ' +
      'when copyright context loads', function(done) {
      store = SpecUtil.mockReduxStore(initialState);

      const copyrightContextUrl = getCopyrightContextUrl('organization', 'org', 'componentHash',
          'componentIdentifier', 'copyright_hash_2', 'path/file.js');
      mockAxiosCalls({
        get: {
          [copyrightContextUrl]:
            Promise.resolve(
      ...
      store.dispatch(loadCopyrightContexts('path/file.js'))
          .then(() => {
            setTimeout(() => {
              ...
              expect(actions[1].payload).toEqual({copyrightContexts: ['context']});
              done();
    ...
    it('immediately dispatches a COPYRIGHT_CONTEXT_REQUEST action and then COPYRIGHT_CONTEXT_FULFILLED action ' +
      'when copyright context unloads', function() {
      store = SpecUtil.mockReduxStore(initialState);
    ...
    it('immediately dispatches a COPYRIGHT_CONTEXT_REQUEST action and then COPYRIGHT_CONTEXT_FAILED action ' +
      'when loading copyright context fails', function(done) {
      store = SpecUtil.mockReduxStore(initialState);
  ...
  describe('load copyright file paths', function() {
    ...
    it('immediately dispatches a COPYRIGHT_FILE_PATHS_REQUEST action and then COPYRIGHT_FILE_PATHS_FULFILLED action ' +
      'when copyright context loads', function(done) {
      store = SpecUtil.mockReduxStore(initialState);

      const copyrightFilePathsUrl = getCopyrightFilePathsUrl('organization', 'org', 'componentHash',
          'componentIdentifier', 'copyright_hash_2', 15, 15);
      mockAxiosCalls({
        get: {[copyrightFilePathsUrl]:
            Promise.resolve(
                {
      ...
      store.dispatch(loadFilePathsOnPageUpdate(1)).then(() => {
        setTimeout(() => {
          ...
          expect(actions[1].payload).toEqual({filePaths: ['path1/file1', 'path2/file2', 'path3/file3']});
          done();
    ...
    it('immediately dispatches a COPYRIGHT_FILE_PATHS_REQUEST action and then COPYRIGHT_FILE_PATHS_FAILED action ' +
      'when loading file paths fails', function(done) {
      store = SpecUtil.mockReduxStore(initialState);

      const copyrightFilePathsUrl = getCopyrightFilePathsUrl('organization', 'org', 'componentHash',
          'componentIdentifier', 'copyright_hash_2', 15, 15);
      mockAxiosCalls({
        get: {[copyrightFilePathsUrl]: Promise.reject(
            {
              value: 'Error'
            })
        }
