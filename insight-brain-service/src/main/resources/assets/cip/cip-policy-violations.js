...
(function() {
	...
	function createPlugin() {
		...
		PolicyViolationTab.prototype.isVisible = function () {
			return !((freemium && !this.options.sampleData) || this.gav.matchState === 'unknown') && Brain.hasFeature('policy-violations');
		};
	...
	(function () {
		...
		policyViolationApp.controller('ViewWaiverController', ['$scope','hudson','$http','$q','PolicyViolationData', function($scope, hudson, $http, $q, policyViolationData) {
			...
			$scope.setupModal = function() {
				...
				$http.get(CLM.path + 'rest/policyWaiver/application/' + policyViolationData.appId + '/component/' + policyViolationData.hash, {
					params : {
						timestamp : new Date().getTime()
					}
				}).success(function (data) {
	...
	function checkAngular() {
		if (Insight && Insight.InformationPanelPlugins) {
			...
			$.each(Insight.InformationPanelPlugins, function (candidateIndex, plugin) {
				if (plugin.name === 'PolicyViolationTab') {
					index = candidateIndex;
				}
			});
			if (index > -1) {
				Insight.InformationPanelPlugins[index] = PolicyViolationTab;
			} else {
