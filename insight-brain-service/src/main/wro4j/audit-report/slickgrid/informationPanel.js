...
(function() {
  ...
  AuditTab.prototype = new Insight.InformationPanelPlugin({priority: 99999});

  AuditTab.prototype.isVisible = function() {
    return this.component.matchState !== 'unknown';
  };
  ...
  AuditTab.prototype.create = function() {
    ...
    if (Brain.hasFeature('component-identifier') &&
      Insight.util.isNotNullOrUndefined(this.component.componentIdentifier)) {
      //CLM server >= 1.13.0 stores audits using componentIdentifier
      $.getJSON(baseAuditUrl,
          'key=' + JSON.stringify({hash: this.component.hash, componentIdentifier: this.component.componentIdentifier}),
        success).error(this.getErrorFn(this.node, this.create, this));
    }
    else if (Insight.util.isNotNullOrUndefined(this.component.groupId)) {
      //CLM server < 1.13.0 stores audits using GAV
      $.getJSON(baseAuditUrl, 'key=' + JSON.stringify(this.component, ['hash', 'groupId', 'artifactId', 'version']),
        success).error(this.getErrorFn(this.node, this.create, this));
    }
    else {
