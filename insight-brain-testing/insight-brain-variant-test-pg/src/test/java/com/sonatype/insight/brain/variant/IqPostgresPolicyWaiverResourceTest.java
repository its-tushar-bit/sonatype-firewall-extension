...
@IqPostgresTest
class IqPostgresPolicyWaiverResourceTest
{
  ...
  @BeforeEach
  void setUp() {
    policyTagDAO = ctx.lookup(PolicyTagDAO.class);
  ...
  private HttpRequest restRequest(OwnerType ownerType, String ownerId) {
    return ctx.restRequest().path(PolicyWaiverResource.RESOURCE_PATH).parameter(ownerType, ownerId);
  }
  ...
  void testGetPolicyWaiversByHash_Application() throws Exception {
    testGetPolicyWaiversByHash(ctx.tempEntity().newApplicationWithParent("PolicyWaiverResourceTest_AppId1"));
  }

  @Test
  void testGetPolicyWaiversByHash_Application_TimeBasedWaivers() throws Exception {
    testGetPolicyWaiversByHash_TimeBasedWaivers(
        ctx.tempEntity().newApplicationWithParent("PolicyWaiverResourceTest_AppId1"));
  }

  @Test
  void testGetPolicyWaiversByHash_Repository() throws Exception {
    testGetPolicyWaiversByHash(ctx.tempEntity().newRepository());
  }

  @Test
  void testGetPolicyWaiversByHash_Repository_TimeBasedWaivers() throws Exception {
    testGetPolicyWaiversByHash_TimeBasedWaivers(ctx.tempEntity().newRepository());
  }
  ...
  private void testGetPolicyWaiversByHash(Owner owner) throws Exception {
    ...
    Policy policy = ctx.tempEntity().newPolicy(grandparent);
    ...
    PolicyWaiver ownerWaiver =
        ctx.tempEntity().newWaiver(hash, policy.getId(), owner.getId(), null, EXACT_COMPONENT, "My comment");
    ...
    PolicyWaiver parentOwnerWaiver =
        ctx.tempEntity().newWaiver(hash, policy.getId(), parent.getId(), null, EXACT_COMPONENT, "My comment");
    ...
    PolicyWaiver grandparentOwnerWaiver =
        ctx.tempEntity().newWaiver(hash, policy.getId(), grandparent.getId(), null, EXACT_COMPONENT, "My comment");
    response = restRequest(owner.getType(), restId).path("component", hash).get();
    ctx.assertResponseStatus(200, response);
    waivers = response.getBody(AppliedWaivers.class);
  ...
  private void testGetPolicyWaiversByHash_TimeBasedWaivers(Owner owner) throws Exception {
    ...
    Policy policy1 = ctx.tempEntity().newPolicy(grandparent);
    Policy policy2 = ctx.tempEntity().newPolicy(grandparent);
    String hash = "12345678901234567890";
    ...
    PolicyWaiver activeWaiver =
        ctx.tempEntity()
            .newWaiver(hash, policy1.getId(), owner.getId(), null, EXACT_COMPONENT,
                "App Scope", now.toDate(), now.plusHours(1).toDate());
    ctx.tempEntity()
        .newWaiver(hash, policy2.getId(), owner.getId(), null, EXACT_COMPONENT, "Expired", null,
            now.toDate());
    HttpResponse response = restRequest(owner.getType(), restId).path("component", hash).get();
    ctx.assertResponseStatus(200, response);
    AppliedWaivers waivers = response.getBody(AppliedWaivers.class);
    ...
    PolicyWaiver activeParentWaiver =
        ctx.tempEntity()
            .newWaiver(hash, policy2.getId(), parent.getId(), null, EXACT_COMPONENT,
                "Parent Scope", now.toDate(), now.plusHours(1).toDate());
    ctx.tempEntity()
        .newWaiver(hash, policy1.getId(), parent.getId(), null, EXACT_COMPONENT, "Expired", null,
            now.toDate());
    response = restRequest(owner.getType(), restId).path("component", hash).get();
    ctx.assertResponseStatus(200, response);
    waivers = response.getBody(AppliedWaivers.class);
    ...
    // Verify grandparent organization level
    PolicyWaiver activeGrandparentWaiver = ctx.tempEntity()
        .newWaiver(hash, policy1.getId(), grandparent.getId(), null, EXACT_COMPONENT,
            "Grandparent Scope", now.toDate(), now.plusHours(1).toDate());
    ctx.tempEntity()
        .newWaiver(hash, policy2.getId(), grandparent.getId(), null, EXACT_COMPONENT,
  ...
  @Test
  void testGetPolicyWaiversByHash_WaiverOnNoLongerApplicablePolicy() throws Exception {
    Organization org = ctx.tempEntity().newOrganization();
    Tag tag1 = ctx.tempEntity().newTag(org.getId());
    Tag tag2 = ctx.tempEntity().newTag(org.getId());
    Application app = ctx.tempEntity().newApplication(org.getId());
    ctx.tempEntity().newApplicationTag(app.getId(), tag1.getId());
    Policy policy = ctx.tempEntity().newPolicy(org);
    PolicyTag policyTag = ctx.tempEntity().newPolicyTag(policy.getId(), tag1.getId());

    PolicyWaiver policyWaiver =
        ctx.tempEntity().newWaiver("hash", policy.getId(), app.getId(), null, EXACT_COMPONENT, "Test Comment");

    // update policy tags so it no longer applies to the application
    policyTagDAO.delete(policyTag);
    ctx.tempEntity().newPolicyTag(policy.getId(), tag2.getId());
  ...
  @Test
  void testGetPolicyWaiversByHash_WithWaiverReason() throws Exception {
    Organization org = ctx.tempEntity().newOrganization();
    Tag tag1 = ctx.tempEntity().newTag(org.getId());
    Application app = ctx.tempEntity().newApplication(org.getId());
    ctx.tempEntity().newApplicationTag(app.getId(), tag1.getId());
    ...
    PolicyWaiver policyWaiver =
        ctx.tempEntity()
            .newWaiverWithReason("hash", policy.getId(), app.getId(), null, "Test Comment",
                "Test Comment", "Reason");
  ...
  void testGetApplicableContexts() throws Exception {
    ...
    Application app = ctx.tempEntity().newApplication(appPublicId, org.getId());
    ...
    policy = ctx.tempEntity().newPolicy(parentOrg);
    response = restRequest(OwnerType.APPLICATION, appPublicId).path("applicable/context", policy.getId()).get();
    ctx.assertResponseStatus(200, response);
    result = response.getBody(ApplicableContext.class);
  ...
  @Test
  void testGetApplicableContexts_Repository() throws Exception {
  ...
  @Test
  void testGetApplicableContexts_PolicyNotApplicable() throws Exception {
    String appPublicId = "testGetApplicableContextsPolicyNotApplicable";
    ctx.tempEntity().newApplicationWithParent(appPublicId);
    Application otherApp = ctx.tempEntity().newApplicationWithParent("otherApp");

    Policy policy = ctx.tempEntity().newPolicy(otherApp);
    HttpResponse response = restRequest(OwnerType.APPLICATION, appPublicId).path("applicable/context", policy.getId())
        .get();
  ...
  @Test
  void testGetApplicableContexts_PolicyNotApplicable_Repository() throws Exception {
    Repository repository = ctx.tempEntity().newRepository();
    Application otherApp = ctx.tempEntity().newApplicationWithParent("anApp");
