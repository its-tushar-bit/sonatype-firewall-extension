...
import com.sonatype.insight.brain.telemetry.TelemetryResource;
...
@IqH2Test
class IqH2TelemetryResourceTest
{
  private IqTestContext ctx;

  @Test
  void testForwardFrontendTelemetryToHds() throws Exception {
    ...
    HttpResponse loginResponse = ctx.restRequest().path("rest/user/session").post();
    HttpCookie sessionCookie = loginResponse.getCookie(SecurityConfiguration.SESSION_COOKIE_NAME);

    HttpRequest request =
        ctx.restRequest().path(TelemetryResource.RESOURCE_PATH).body(postData).cookie(sessionCookie);
