...
describe('access.editor.controller.spec.js', function () {
  ...
  it('Sets roles and originalMembers', function () {
    ...
    $httpBackend
      ...
      .respond(accessMockData.getMoreRoleMappings());
    $httpBackend.flush();
  ...
  it('Calls remove on update with no picked users', function () {
    inject(function ($controller) {
      ...
      vm.getCurrentMembersToSave = function () {
        return [];
      };
      vm.isMembershipDirty = function () {
        return true;
      };
    });
    $httpBackend
  ...
  it('Remove frees the role, broadcasts update and transfers to create new', function () {
    ...
    $timeout.flush();
    expect(vm.availableRoles.length).toBe(1);
    expect(mockRootScope.$broadcast).toHaveBeenCalledWith(
  ...
  it('Adding the last role removes it from available, broadcasts update and transfers to edit', function () {
    ...
    vm.getCurrentMembers = function () {
      return [{ internalName: 'testUser', picked: true }];
    };
    vm.getCurrentMembersToSave = function () {
      return [{ internalName: 'testUser' }];
    };
    vm.accessEditorMask = { wrap: SpecUtil.promiseWrapper($q) };

    vm.save();
    $httpBackend
      .expectPUT(CLMContextLocations.getRoleMappingUrl(vm.role.roleId))
      .respond(200);
    $httpBackend.flush();
    ...
    expect(
      mockSameOwnerStateNavigationService.goEdit
    ).toHaveBeenCalledWith('edit-access', { roleId: 'abcdef' });
  });
