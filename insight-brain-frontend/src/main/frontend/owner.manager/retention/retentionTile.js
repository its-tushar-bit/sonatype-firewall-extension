...
function RetentionTileController(CLMContextLocations, $scope, EventNameConstant, OrganizationStore, retentionService,
                                 $q, Messages) {
  ...
  Object.assign(vm, {
    ...
    getMaxReports(applicationReport) {
      return applicationReport.enablePurging ?
        (applicationReport.maxCount ? applicationReport.maxCount : NOT_APPLICABLE) : NOT_ENABLED;
    },
...
RetentionTileController.$inject = [
  'CLMContextLocations', '$scope', 'event.name.constant', 'OrganizationStore', 'retentionService', '$q', 'Messages'
];
