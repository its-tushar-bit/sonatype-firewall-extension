    ...
    extends AbstractIqUiTest
{
  ...
  private void awaitNoExecutingSchedulerJobs() {
    ...
    Awaitility.await().atMost(indexServiceJobInterval()).until(() -> {
      try {
  ...
  // Reads IndexService.JOB_EXECUTION_INTERVAL_IN_SECONDS reflectively so the timeout tracks
  // any future change to that constant without requiring a manual update here.
  private static Duration indexServiceJobInterval() {
    try {
      Field f = IndexService.class.getDeclaredField("JOB_EXECUTION_INTERVAL_IN_SECONDS");
      f.setAccessible(true);
      return Duration.ofSeconds((int) f.get(null));
    }
    catch (Exception e) { // covers ReflectiveOperationException + InaccessibleObjectException (Java 25 modules)
