...
@IqH2Test
class IqH2ApiRepositoryPathResourceTest
{
  private IqTestContext ctx;

  @Test
  void testGetRepositoryPaths() throws Exception {
    Repository repository = ctx.tempEntity().newRepository("repositoryManager1", "repo1", "npm");
    ctx.tempEntity()
        .newRepositoryComponent(repository.getId(), MatchState.EXACT, "comp1/-/comp1-1.tgz", "hash1-1",
            ComponentIdentifier.createNpmCoordinates("comp1", "1"), true);

    HttpResponse response =
        ctx.restRequest()
            .path(PublicApiPaths.REPOSITORIES_RESOURCE_PATH + "/" + ApiRepositoryPathResource.PATHNAMES_PATH)
