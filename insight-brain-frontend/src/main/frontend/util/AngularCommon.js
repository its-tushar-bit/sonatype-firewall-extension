...
angularCommon.controller('DeleteResourceController', [
  '$scope',
  '$http',
  'CLMContextLocations',
  'selected',
...
angularCommon.directive('clmButtons', [
  function () {
    return {
      scope: {
        acceptFn: '&',
      ...
      template:
        '<button class="btn btn-link btn-cancel" type="button" ng-click="decline()" ' +
...
angularCommon.directive('loadError', [
  ...
  function (messages) {
    return {
      ...
      priority: 99,
      template:
        '<div ng-show="error != null" class="iq-alert iq-alert--error">' +
...
angularCommon.directive('clmAlerts', function () {
  return {
    restrict: 'A',
    template:
      '<div class="alert {{ \'alert-\' + (alert.type || \'warning\') }}" ng-repeat="alert in alerts">' +
...
angularCommon.directive('focusInput', [
  '$parse',
  function ($parse) {
    return {
      require: '?^form',
      ...
      link: function (scope, element, attrs) {
        ...
        attrs.$observe('clmInclude', function (val) {
          if (val) {
            ...
            $http.get(val, { cache: $templateCache }).then(
              function (response) {
...
angularCommon.directive('chiclets', function () {
  return {
    ...
    template:
      '<span ng-show="critical || alwaysShow" class="{{baseClass}}" ng-class="{\'critical\': critical }" ' +
...
angularCommon.directive('hexagon', [
  function () {
    return {
      ...
      replace: true,
      template:
        '<svg class="hexagon" viewBox="0 0 185.5 208" preserveAspectRatio="xMidYMid meet">' +
        // eslint-disable-next-line max-len
 ...
 */
angularCommon.directive('autofill', [
  '$timeout',
  function ($timeout) {
    return {
      require: 'ngModel',
...
angularCommon.directive('breadcrumb', [
  '$state',
  'state.history.service',
  function ($state, StateHistoryService) {
    ...
    return {
      ...
      template:
        '<p class="nav-crumb"><span ng-repeat="state in states">' +
        ...
        config = angular.extend(
          {
            ...
            keyboard: false,
            template:
              '<div id="{{id}}"><div class="iq-modal-header"><h2>{{title}}</h2></div>' +
...
angularCommon.service('ErrorDialog', [
  'Dialog',
  'Messages',
