...
describe('evaluate.application.modal.controller.spec.js', function() {
  ...
  describe('Bundle submit', function() {
    ...
    it('Test submit failure', function() {
      $httpBackend.expectPOST(CLMLocations.getBundleUploadUrl(mockSelectedApplication.publicId,
          'release', 'true')).respond(500, 'Some failure');
    ...
    it('Test evaluation polling loop', function() {
      ...
      $timeout.flush();
      $httpBackend.flush();
      expect(vm.pollingUrl).toEqual(CLMLocations.getEvaluationStatusUrl(mockSelectedApplication.publicId,
          'ticket'));
    });
