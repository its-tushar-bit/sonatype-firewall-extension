...
(function() {
  'use strict';
  var compInfoExtLoaded = false;
  var compInfoScope = null;
  function panelLoadHandler(event, data) {
    //we only want to load this stuff once, the angular model/controller definitions
    if (!compInfoExtLoaded) {
      compInfoExtLoaded = true;
              ...
              [
                  ...
                  function($scope, $http, $q) {
                    $scope.viewWaivers = function() {
                      ...
                      $q.all([policyWaiverPromise, policyPromise]).then(
                              function(results) {
                                ...
                                $.each($scope.waivers, function(waiverIndex, waiver) {
                                  ...
                                  waiver.createTimeStr = (date.getMonth() + 1) + '/' + date.getDate() + '/'
                                          + date.getFullYear();
    ...
    //when the button is clicked, get data in the scope and $apply since we are outside of angular
    ...
    //only redefine the modal if not existing
    if (!$('#componentExistingWaiverModal').length) {
      $('body')
              .append(
  ...
  //clear handler if already set and add a new one
